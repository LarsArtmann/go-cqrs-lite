...
Multi-module Go workspace (`go.work`) with 71 `go.mod` files — all wired into `go.work`. Breakdown: 51 library + 10 stack presets + 3 examples + 5 cmd + 1 root workspace + 1 eventtest nested + 1 external. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
