...
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Language  | Go 1.26.3                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Modules   | `event`, `command`, `query`, `decider`, `deriver`, `id`, `id/idtest`, `idempotency`, `query/querytest`, `dispatcher`, `schema`, `snapshot`, `catalog`, `middleware`, `integration`, `storage`, `storage/memory`, `storage/pebble`, `storage/turso`, `signing`, `encryption`, `otel`, `prometheus`, `watermill`, `transport/http`, `transport/grpc`, `codec`, `kv`, `kv/viewstoretest`, `listing`, `graph`, `projection`, `projectionhost`, `scenario`, `scheduling`, `testutil`, `cqrs-gen`, `api-stability`, `doc-check`, `stack`, `stack/memory`, `stack/sqlite`, `stack/turso`, `stack/pebble`, `stack/postgres`, `stack/bench`, `example/user`, `example/todo`, `example/encryption`, `example/deployer-first`, `example/deployer-first-multidb`, `example/deployer-first-heterogeneous`, `example/graph-demo`, `example/deriver`, `example/projectionhost`                                                                                                                                              |
| Build     | `nix run .#build`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Test      | `nix run .#test` or `go test ./event/... ./command/... ./idempotency/... ./query/... ./decider/... ./deriver/... ./id/... ./dispatcher/... ./schema/... ./snapshot/... ./storage/memory/... ./catalog/... ./middleware/... ./integration/... ./signing/... ./encryption/... ./storage/... ./storage/pebble/... ./storage/turso/... ./watermill/... ./transport/http/... ./transport/grpc/... ./codec/... ./kv/... ./listing/... ./projection/... ./projectionhost/... ./scenario/... ./scheduling/... ./testutil/... ./cmd/cqrs-gen/... ./cmd/doc-check/... ./prometheus/... ./otel/... ./stack/... ./stack/memory/... ./stack/sqlite/... ./stack/turso/... ./stack/pebble/... ./stack/postgres/... ./stack/bench/... ./example/user/... ./example/todo/... ./example/encryption/... ./example/deployer-first/... ./example/deployer-first-multidb/... ./example/deployer-first-heterogeneous/... ./example/deriver/... ./example/graph-demo/... ./example/projectionhost/... ./graph/... -count=1` |
| Lint      | `nix run .#lint`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Format    | `nix fmt`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Multi-module Go workspace (`go.work`) with 54 `go.mod` files — all wired into `go.work`. Breakdown: 34 library + 7 stack presets + 9 examples + 3 cmd + 1 integration. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
├── projectionhost/      # Managed projection host: Host, Worker, DeadLetterStore — composes journal + projection + checkpoint + DLQ into a crash-restart lifecycle (framework gap A1, ADR-0030)
...
├── example/deployer-first-heterogeneous/ # Multi-store heterogeneous deployer-first (graph + relational + KV)
├── example/deriver/    # Event→command derivation: welcome email + CRM sync from user.created
├── example/projectionhost/ # Managed projection host: DLQ capture + replay end-to-end
...
//   for _, s := range host.Status() {  // health: idle/running/backoff/draining/stopped/failed
...
Layer 3: decider/ (→event, snapshot), graph/ (→event, id, projection), scenario/ (→event, id, projection), projectionhost/ (→event, id, projection)
