...
22. **Temporal-composability contract (ADR-0136, 2026-09-10)** — every effect sits on the invertibility ladder: replayable (derived data → `Reset` + replay; enforced by `projectionhost.Resettable` + `metaengine.EngineResetter`/`Store.Reset`, warn-first v4.x, hard v5) → compensable (external effects → `deriver` sagas, never `Close()`) → must-be-an-event (facts → tombstone/rebirth per ADR-0114). Ask "what is its inverse?" before writing an effect. Reset must be total or loud (`ResetResult.Partial()`); memory engine implements `EngineResetter`, persistent engines are follow-ups (sqliteengine prioritized).
23. **Health-driven engine deactivation (ADR-0137, 2026-09-10)** — `metaengine` quarantines an engine after N consecutive errorfamily Infrastructure/Transient failures (default 3, `SetEngineFailureThreshold`); execution reroutes via `effectiveQueryLocked` (execution-scoped `routedQuery` override — plan untouched) and `routableLocked` never re-plans onto quarantined engines. Reactivation requires a successful `Prober` probe (`StartAutoReprobe`) or explicit `ReactivateEngine` — never a timeout. Lock ordering: `s.mu` → `healthMu` only; health methods never take `s.mu`. Rejection/Conflict/Corruption/unclassified errors never count toward health. Reads reroute; writes to a quarantined engine fail loudly (fold-write failover is a TODO_LIST follow-up).
