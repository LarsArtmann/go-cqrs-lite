...
| `metaengine/adttest/`                                          | Exported ADT test harness (RunMatrix)                                                              | Package INSIDE metaengine module, not a module    |
...
- **Soak test env vars**: `SOAK_SKIP_10M=1` skips 10M-event soak (~5s/25s-race). `SOAK_SKIP_DUCKDB=1` skips DuckDB AutoCRUD soak (~80s/100s-race). The 50K-event `TestSoak_MemoryBounded` always runs as smoke.
