...
- **Multi-module tag waves have three hard mechanics** (2026-08-22 wave: 8 tags, 66-module pin sweep): (1) `go mod tidy` does NOT bump require pins — it resolves at PACKAGE level, so a pin on an older tag that lacks new SYMBOLS only fails at BUILD time; pre-bump every dependent module's pin (`go mod edit -require=...@<tag>`) BEFORE running `scripts/tag-release.sh`, or its standalone-build verify aborts the tag. (2) `GOPRIVATE=github.com/larsartmann/*` means siblings resolve via DIRECT VCS fetch from origin — a tag must be PUSHED before any dependent module's tag-time tidy can see it; interleave cut→push→next. (3) A published consumer of a changed API can strand OTHER consumers: published storage v4.6.0's timer twin predated branded TimerID, so consumers bumping scheduling v4.3.0 failed standalone until storage was re-tagged v4.8.0 and their storage pins bumped — always run the GOWORK=off build matrix over ALL swept modules after a wave. The api-stability golden is unaffected by go.mod sweeps, but cqrs-lint's taskmanager golden pins the version set (V006) — refresh it in the same wave.
