...
26. **Vector semantics are load-bearing across engines (2026-09-15)** — `metaengine.VectorDistance` contract: cosine = `1-cosSim`, dot = NEGATED dot, euclidean = L2; ascending sort = nearest (`TopKNearest`). Engine-native functions were verified to match EXACTLY: libSQL `vector_distance_cos/l2/dot` (⚠ `vector_distance_dot` already returns the negated dot — identical vectors → -1; do NOT negate again), DuckDB `array_cosine_distance`/`array_negative_inner_product`/`array_distance` (⚠ core `array_*` functions only bind fixed `FLOAT[n]`, not `FLOAT[]` lists — cast both sides with the query dimension formatted into the SQL text). SQL engines store vectors as raw LE float32 via `metaengine.EncodeVectorF32` (F32_BLOB-compatible, no header). modernc.org/sqlite cannot load C extensions (no `LoadExtension` on the database/sql surface in v1.58.0) — sqlite-vec stays an operator option, not a library dep.
