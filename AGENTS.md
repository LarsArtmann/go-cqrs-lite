...
Multi-module Go workspace (`go.work`) with 60 `go.mod` files — all wired into `go.work`. Breakdown: 42 library + 9 stack presets + 3 examples + 5 cmd + 1 root workspace. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
├── metaengine/          # Cost-based storage planner — THE STRATEGIC FUTURE of this project (possibly a future dedicated project). Engine, Store, Plan, 7 ADTs, cost model, SQLite engine ([ADR-0061](docs/adr/0061-metaengine-sqlite-engine.md), [ADR-0062](docs/adr/0062-metaengine-dependency-boundary.md), [ADR-0063](docs/adr/0063-metaengine-pushdown.md)). ⚠️ CANONICAL DESIGN DOCS (read before working on metaengine): [project-definition](docs/planning/meta-engine-project-definition.md), [design/vision](docs/planning/meta-engine-design.md), [assumptions & query-planning](docs/planning/meta-engine-assumptions-and-query-planning.md). Zero production deps in core. SQLite engine hardened: tx-atomic MapUpdate (no lost concurrent updates), restart-safe multimap seq (sync.Once MAX(seq) seed), ExecuteTyped reifies map[string]any→struct across engines. Caller owns the *sql.DB (engine Close is a no-op). **Query pushdown** (Phase 1): FilterOnField/SortOnField declarative specs → PushdownScan interface → SQLite json_extract() WHERE/ORDER BY pushdown (O(N)→O(logN) for SortedMap). **Layout planning** (Phase 3, [ADR-0073](docs/adr/0073-metaengine-layout-planning.md)): LayoutPlan generates deployment-time DDL from declared query patterns — planned tables use indexed columns instead of json_extract (10x faster on filter+sort). Auto-applied by Plan() via LayoutPlanner interface when query uses FilterOnField/SortOnField. RawValueReader/RawScanReader optional interfaces enable single-pass JSON decode (1 op instead of 3). TypedReader[V] provides typed Get/Scan/Exists without ExecuteTyped. Exported error sentinels: ErrNotFound, ErrAmbiguousKey, ErrUnsupportedADT, ErrLayoutConflict. **Streaming reads** (Phase 5): StreamScan(ctx) iter.Seq2 for OOM-safe lazy iteration. FilterOp/FilterSpec/SortSpec types. **SSE reconnection**: Watcher.WithReplay(capacity) attaches an SSEReplay[V] ring buffer with monotonic seq counter; ServeSSE writes `id: <seq>` on every event and replays missed values from the journal on reconnect via `Last-Event-ID` header with dedup (WithSSEReplayLimit caps backlog). WatchWithSeq returns SeqValue[V] pairs (seq+value). **Cursor-encoded prefetch**: prefetchCursorKey uses Cursor.Encode() (base64+JSON) for HTTP-safe opaque cache keys; WithCursorString(s) parses an encoded cursor string so both WithCursor(raw) and WithCursorString(encoded) produce matching PrefetchCache keys.
...
├── cmd/cqrs-lint/       # Domain-aware linter: 159 rules across 10 categories (correctness, API misuse, boilerplate, consistency, architecture, security, performance, version, testing, adoption). Built on go-finding + cmdguard. CLI: struct-tag flags, config file (.cqrs-lint.json), --min-confidence, --health-score, --verbose, --color, SARIF/JSON/markdown output. Feature profile system: auto-detects which go-cqrs-lite modules a consumer uses (store, command-flow, server, soft-delete, tracing, snapshot) and adapts context-dependent rules. `cqrs-lint doctor` prints the detected profile. Config presets (local-cli, production, library, read-only) as sugar over feature flags. F-series adoption rules proactively coach users toward unused features (tombstone soft-delete, catalog docs, OTel, Prometheus, encryption, CBOR, scheduling, graph/relational projections, deriver, transport, kv.Cache, metaengine, listing, dedup). D-series consistency rules detect API inconsistencies (mixed event.New/NewEvent, mixed codec decode, mixed io.Closer/anonymous interface, generic "internal" error codes, missing schema version stamping). E-series architecture rules detect consumer design issues (stack preset bypass, missing HTTP integration, capture without domain validation, excessive adapter layers, dual-write without completion, signing disabled by default, no read-your-writes, ordered delivery disabled)
...
| Category   | Packages                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Production | oklog/ulid/v2, go-branded-id, go-error-family, go-faster/yaml (catalog); go.opentelemetry.io/otel (otel, event, storage, middleware, prometheus); prometheus/client_golang (prometheus); golang.org/x/crypto (encryption); fxamacker/cbor/v2 (codec); maypok86/otter/v2 (kv, decider); failsafe-go/failsafe-go (middleware); modernc.org/sqlite (storage, stack/sqlite — pure-Go); github.com/jackc/pgx (storage, stack/postgres); github.com/duckdb/duckdb-go (stack/duckdb — CGo, C++ engine); cockroachdb/pebble (storage/pebble) |
| Test-only  | onsi/ginkgo/v2, onsi/gomega, pgregory.net/rapid (event, encryption); testcontainers-go/modules/postgres (stack/postgres, storage, benchkit); go-snaps (eventtest, catalog, otel, codec — golden/snapshot testing)                                                                                                                                                                                                                                                                                                                    |

**Coverage** (verified 2026-07-27 via `go test -tags "goexperiment.jsonv2" -cover`, workspace mode): core modules (decider 96.9%, storage/memory 94.4%, schema 89.9%, command 88.3%, event 88.3%, id 86.4%); mid-tier (snapshot 91.9%, metaengine 86.1%, dispatcher 81.5%, query 80.0%); newer modules (kv 71.9%, codec 70.2%). Bundle layer (stack presets, cache) 0–87% — presets emphasise the shared contract suite + happy paths. stack/postgres tests now run locally via testcontainers (was 0% when skipping without POSTGRES_TEST_DSN). Coverage drift is checked by `scripts/check-coverage.sh` (run via `nix run .#check-coverage`). See `docs/status/` for latest.
...
Tier 5 — Composition: stack/, stack/memory/, stack/sqlite/, stack/duckdb/, stack/pebble/, stack/postgres/, stack/turso/
