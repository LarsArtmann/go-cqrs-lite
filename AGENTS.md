...
Multi-module Go workspace (`go.work`) with 56 `go.mod` files — all wired into `go.work`. Breakdown: 40 library + 7 stack presets + 3 examples + 5 cmd + 1 root workspace. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
├── benchkit/            # Factory-driven benchmarking suite: Run/Compare, latency percentiles, throughput, memory/disk (mirrors contracttest pattern)
...
> depends on event/, and 38 of 56 modules depend on codec/. The four-tier model reflects reality.
