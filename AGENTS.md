...
Multi-module Go workspace (`go.work`) with 64 `go.mod` files — all wired into `go.work`. Breakdown: 46 library + 9 stack presets + 3 examples + 5 cmd + 1 root workspace. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
│   └── duckdbengine/   # DuckDB-backed metaengine Engine (columnar OLAP, CGo). MapBackend, CounterBackend, ScanBackend, **PushdownScan** (json_extract filter/sort pushdown), **LayoutPlanner** (planned tables with extracted columns + ART indexes). Cross-engine parity via adttest.RunMatrix. Separate module (duckdb-go dep, CGo required)
