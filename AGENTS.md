...
Multi-module Go workspace (`go.work`) with 65 `go.mod` files — all wired into `go.work`. Breakdown: 47 library + 9 stack presets + 3 examples + 5 cmd + 1 root workspace. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
├── irohengine/           # Iroh Level 2 replication wrapper: Replicated(localEngine, ...) adds CRDT convergence via pluggable Transport. In-process Network mock simulates P2P sync today; swap for real Iroh FFI Transport when iroh-docs C bindings mature ([ADR-0096](docs/adr/0096-iroh-distributed-engine-bridge-evaluation.md)). CRDT-safe: MapSet (LWW), SetAdd (OR-Set), CounterIncrement (PN-Counter), MultiAdd, LogAppend. Non-CRDT ops (MapUpdate) stay local.
