...
| Verify CI     | `nix run .#verify-ci` (GOWORK=off per-module build+test — mirrors the CI matrix job; runs `check-go-version` first)                                                                                       |
| Verify-lock   | advisory machine-local flock (`scripts/lib/verify-lock.sh`): consumers today are `tag-release.sh` + `batch-release.sh` only (tree-WRITING release runs). `#verify`/integration suites coordinate via `can-run-composed-gate` + their own load gates instead; read-only chains (smoke-all checks, load-sweep) deliberately take NO lock — the lock serializes tree writers, not readers. `VERIFY_LOCK=0` is the documented escape hatch. |
...
1. **Max 350 lines/file, 30 lines/function** — enforced by the baseline+ratchet gate (`nix run .#check-file-size`): 58 historical offenders are baselined in `scripts/file-size-baseline.txt`; NEW offenders and GROWTH of baselined files fail CI, shrinking is allowed. `--update-baseline` only on structural shifts.
...
14. **Dedup helper patterns** — `storage/memory` uses `withWriteLock(code, msg, fn)` + `withReadLock[T](s, code, msg, fn)` + `wrapClosed(err, code, msg)`. the discard-close idiom is `record.DeferClose` (Tier 0, ADR-0144, canonical address); `metaengine.DeferClose(c Closer)` stays as the self-contained engine-tier twin (forwarding was rejected: it would force a `record` sibling replace into every metaengine-replace consumer). Together they replaced `defer func() { _ = x.Close() }()` across all engine modules (72 production + 33 test sites, 2026-09-10 recount) and the 2026-09-20 sweep finished the tail (kv, storage, storage/pebble, storage/turso/indexing, scheduling/sqlstore, projectionhost, stack, benchkit, examples); bare `defer x.Close()` (C015-exempt) and rollback-defers are NOT conversion targets. The `.art-dupl-baseline.json` golden + `nix run .#check-duplication` gate enforce no-new-clones; run `art-dupl baseline . --threshold 3 --semantic` to update after a consolidation. A `//art-dupl:accept <reason>` comment on/above a clone region suppresses that group LIVE — annotate intentional clones instead of re-pinning the baseline; reserve baseline regen for structural shifts. Annotation is ITERATIVE: art-dupl reports one group per region pair, so suppressing the visible groups unmasks others behind them — re-run until "0 new clone groups" (2026-08-16: 12 groups took 4 rounds). The directive must sit directly on/above the region's FIRST line; placing it above the following function's doc comment does not suppress. The `#check-duplication` app refuses to run while `.art-dupl-baseline.json` has uncommitted changes (dirty-tree guard: re-pins must happen on a committed baseline).
...
25. **Lifecycle rejections partition failures (2026-09-15, T17 option A)** — `commandlifecycle` emits `command.rejected` (payload stamps family + code) for errors whose family is in `DefaultRejectionFamilies` {Rejection, Conflict}, overridable via `WithRejectionFamilies`. Rejections are NOT dead-lettered and NOT in `FailureLog` (the DLQ is ops-only; `RejectionLog` folds rejected). Non-rejection flows are unchanged. Changing the contract or `projections.All()` touches every `system.WithCommandLifecycle` consumer — run the system module tests.

26. **Vector semantics are load-bearing across engines (2026-09-15)** — `metaengine.VectorDistance` contract: cosine = `1-cosSim`, dot = NEGATED dot, euclidean = L2; ascending sort = nearest (`TopKNearest`). Decision record: [ADR-0140](docs/adr/0140-vector-distance-semantics-contract.md). Engine-native functions were verified to match EXACTLY: libSQL `vector_distance_cos/l2/dot` (⚠ `vector_distance_dot` already returns the negated dot — identical vectors → -1; do NOT negate again), DuckDB `array_cosine_distance`/`array_negative_inner_product`/`array_distance` (⚠ core `array_*` functions only bind fixed `FLOAT[n]`, not `FLOAT[]` lists — cast both sides with the query dimension formatted into the SQL text). SQL engines store vectors as raw LE float32 via `metaengine.EncodeVectorF32` (F32_BLOB-compatible, no header). modernc.org/sqlite cannot load C extensions (no `LoadExtension` on the database/sql surface in v1.58.0) — sqlite-vec stays an operator option, not a library dep. Every engine's `VectorInsert` enforces the first-insert-wins DIMENSION LOCK (`metaengine.CheckVectorDimension`, `ErrVectorDimensionMismatch` — Rejection family; `adttest.AssertVectorDimensionGuard` pins it per engine), and engines report their execution path via `metaengine.VectorPathReporter` (`sql-pushdown`/`go-scan`), rendered in ExplainPlan/Doctor; the sqlite libSQL probe is LAZY (`sync.OnceValue` on first vector use, not construction).
27. **Engine shared plumbing lives in metaengine core with label-prefix errors (2026-09-23, t4 clone campaign)** — when SQL-engine code repeats across duckdb/sqlite/mysql/pg engines, the shared mechanics go into `metaengine` core (precedent: `QuoteIdent`, `DeferClose`, `SQLExec`, `EncodeVectorF32`): the scan family (`ScanKeyValuesPage`, `ScanLimit`, `CursorArg`, `ScanGroupedAggregates`, `ScanVectorResults`), the vector insert family (`ScanVectorDimensionProbe`, `VectorMetadataArg`), planned tables (`ListPlannedTables` — the CALLER owns the lock mode: duck RLock, pg Lock, sqlite none), filter clauses (`AppendPlannedFilter` + `QuestionPlaceholders`/`DollarPlaceholders`), and `GraphBFS` (iterative BFS fallback for servers without WITH RECURSIVE). Helpers take a `label string` (e.g. `"duckdbengine.VectorSearch"`) and return the FULLY-wrapped error — callers never double-wrap; error strings stay byte-identical across engines, and `VectorMetadataArg` uses `encoding/json/v2` deliberately (v1 HTML-escapes; byte-identical storage). Engine-side residue after extraction (the dimension-lock call ladder) is accept-annotated, not merged: probe SQL, exec surfaces, and transaction routing (`e.conn(ctx)` vs `e.db`) are dialect territory.
