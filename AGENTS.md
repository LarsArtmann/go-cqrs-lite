...
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Language   | Go 1.26.4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Modules    | `event`, `command`, `command/commandtest`, `query`, `decider`, `deriver`, `id`, `id/idtest`, `idempotency`, `idempotency/kvstore`, `idempotency/sqlstore`, `metadata`, `query/querytest`, `dispatcher`, `schema`, `snapshot`, `catalog`, `middleware`, `integration`, `storage`, `storage/eventstore`, `storage/readmodel`, `storage/memory`, `storage/pebble`, `storage/bbolt`, `storage/turso`, `signing`, `encryption`, `otel`, `prometheus`, `watermill`, `transport/http`, `transport/grpc`, `codec`, `kv`, `kv/viewstoretest`, `listing`, `graph`, `projection`, `projectionhost`, `scenario`, `scheduling`, `scheduling/sqlstore`, `testutil`, `testutil/pgtestcontainer`, `dedup`, `retry` (DEPRECATED), `flightrecorder`, `benchkit`, `cqrs-gen`, `cqrs-lint`, `cqrs-bench`, `api-stability`, `doc-check`, `stack`, `stack/memory`, `stack/sqlite`, `stack/duckdb`, `stack/turso`, `stack/pebble`, `stack/bbolt`, `stack/postgres`, `stack/mysql`, `stack/bench`, `example/taskmanager`, `example/getting-started`, `example/readme-quickstart`, `example/metaengine-quickstart`, `metaengine`, `metaengine/bench`, `metaengine/adttest`, `metaengine/pebbleengine`, `metaengine/duckdbengine`, `metaengine/pgengine`, `metaengine/irohengine`, `metaengine/irohengine/loopback`, `metaengine/irohengine/quic`, `metaengine/projectionadapter`, `metaengine/sqliteengine`, `metaengine/badgerengine`, `metaengine/dgraphengine`, `metaengine/graphadapter`, `metaengine/keycodec`, `metaengine/enginetest`, `record`, `system` |
| Build      | `nix run .#build`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Test       | `nix run .#test` or `go test ./event/... ./command/... ./idempotency/... ./idempotency/kvstore/... ./query/... ./record/... ./decider/... ./deriver/... ./id/... ./metadata/... ./dispatcher/... ./schema/... ./snapshot/... ./storage/memory/... ./catalog/... ./middleware/... ./integration/... ./signing/... ./encryption/... ./storage/... ./storage/pebble/... ./storage/bbolt/... ./storage/turso/... ./watermill/... ./transport/http/... ./transport/grpc/... ./codec/... ./kv/... ./listing/... ./projection/... ./projectionhost/... ./scenario/... ./scheduling/... ./scheduling/sqlstore/... ./testutil/... ./testutil/pgtestcontainer/... ./dedup/... ./retry/... ./flightrecorder/... ./benchkit/... ./cmd/cqrs-gen/... ./cmd/cqrs-lint/... ./cmd/cqrs-bench/... ./cmd/doc-check/... ./prometheus/... ./otel/... ./stack/... ./stack/memory/... ./stack/sqlite/... ./stack/duckdb/... ./stack/turso/... ./stack/pebble/... ./stack/bbolt/... ./stack/postgres/... ./stack/mysql/... ./stack/bench/... ./example/taskmanager/... ./example/getting-started/... ./example/readme-quickstart/... ./example/metaengine-quickstart/... ./graph/... ./metaengine/... ./metaengine/pebbleengine/... ./metaengine/duckdbengine/... ./metaengine/pgengine/... ./metaengine/irohengine/... ./metaengine/projectionadapter/... ./metaengine/sqliteengine/... ./metaengine/badgerengine/... ./metaengine/dgraphengine/... ./metaengine/graphadapter/... ./system/... ./idempotency/sqlstore/... -count=1`                            |
| Lint       | `nix run .#lint`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Format     | `nix fmt`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| Int. All   | `nix run .#test-integration` (auto-detect best strategy: ephemeral/nspawn/testcontainers/VM) or `nix run .#integration-all` (ephemeral PG + nspawn MySQL) or `nix run .#test-all-backends` (SQLite+Pebble+bbolt+DuckDB+PG+MySQL) or `nix run .#verify-integration` (integration gate only)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
│                        # **DX helpers** (dsl.go): NewSQLiteEngineFromDSN(dsn) one-call SQLite setup (PRAGMA WAL+busy_timeout, MaxOpenConns(1), table creation — replaces 25 lines of boilerplate). PlanFromSQLite(dsn, queries...) one-shot Memory+SQLite plan (replaces 30+ lines). Store.LogPlan(logger) logs planner decisions via slog (replaces 19-line logging loop). projectionadapter: EventWithID[P] exported type (replaces consumer-reinvented wrapper), NewTypeDecoder(Register(...)) replaces 70+ line decoder switch/case, NewWithDecoder clean constructor (no nil+override).
│   └── pebbleengine/   # Pebble-backed metaengine Engine (LSM point reads, 7x faster than SQLite on MapGet). MapBackend, ScanBackend, SetBackend, CounterBackend, MultimapBackend, LogBackend. **RawValueReader + RawScanReader** (eliminates JSON decode tax on point lookups and filtered scans). Separate module (cockroachdb/pebble dep)
...
│   └── bench/          # Cross-engine benchmark module: M4.2 DuckDB columnar extraction (3-way: WithColumnarLayout vs json_extract pushdown vs Memory), parity tests (Memory/SQLite/DuckDB/Pebble), planner/layout/materialize benchmarks. Imports ALL engines (breaks Tier 0 dep boundary — separate module following stack/bench precedent). CGo required for DuckDB benchmarks.
...
│   └── sqliteengine/     # SQLite-backed metaengine Engine: extracted from core (ADR-0115). tx-atomic MapUpdate, restart-safe multimap seq, ExecuteTyped reifies map[string]any→struct. Full SQL pushdown + layout planning.
│   └── badgerengine/     # Badger-backed metaengine Engine: LSM point reads, all 8 core ADTs pass adttest.RunMatrix. Implements Calibratable (ADR-0118).
│   └── dgraphengine/    # Dgraph-backed metaengine Engine: distributed graph DB. MapBackend, CounterBackend, ScanBackend, GraphBackend (O(degree^depth), NOT degraded), SetBackend, SearchBackend (@index(term), NOT degraded). Pure Go via dgo v240 gRPC client.
│   └── graphadapter/     # Graph adapter: wraps graph.MemoryDriver as metaengine Engine for traversal-heavy read models.
│   └── keycodec/        # Key encoding/decoding utility: composite key construction for LSM-style backends (Badger, Pebble). Shared prefix/range helpers.
│   └── enginetest/     # Exported engine test harness: RunMatrix, RunTransactionalTest, RunStreamLogBackendTest, RunAtomicAppenderTest — imported by engine modules for parity.
├── record/              # Shared Record + CommonMetadata types — structural base for both Events and Commands (ADR-0111). Zero deps. event.AsRecord() adapter bridges the ES pipeline.
...
│   └── loopback/         # LoopbackTransport: real TCP connections with length-prefix framing (NO CGo). Middle tier of transport testing pyramid: catches serialization/framing bugs that InProcessNetwork cannot, runs in CI without Rust toolchain. 10 convergence tests (incl. Multimap) + ADT matrix pass with -race.
│   └── quic/             # QuicTransport: real Iroh QUIC streams via iroh-go CGo bindings (pre-compiled static lib, needs gcc only). Real NAT traversal, real RTT from conn.Rtt() ACK timing, ticket-based bootstrap, relay mode for star topology. 10 convergence tests (incl. Multimap) + ADT matrix + 2 reconnect tests pass with -race. CBOR type normalization preserves Go int across wire. dedup.Ring for O(1) op dedup. slog.Warn on OpenBi failures. Separate module (CGo isolation).
...
│   └── pgtestcontainer/ # Shared Postgres testcontainer helpers: per-test database isolation, DATABASE_URL/POSTGRES_TEST_DSN override.
├── dedup/               # Bounded dedup ring buffer: Ring, DefaultCapacity (O(1) fixed-capacity ID dedup for stream boundaries)
├── retry/               # DEPRECATED: re-export aliases for github.com/larsartmann/go-retry — import go-retry directly. Kept for backward compat ([ADR-0064](docs/adr/0064-extract-retry-module.md))
...
├── example/metaengine-quickstart/ # Metaengine quickstart: Record-aware auto-projection example
...
12. **Dependency budgets** — Per-module direct PRODUCTION dep limits enforced by `nix run .#check-arch` (Layer 1 cross-module rules). Test-only packages (gomega, ginkgo, rapid) are excluded from the count. Adding production deps requires explicit budget review.
...
- **Nix-based integration tests (no Docker needed)** — three approaches, all using nixpkgs services pinned by flake.lock:
  - **Ephemeral PG** (`nix run .#integration-pg`): starts a `pg_ctl` process from nixpkgs in a temp dir, runs all PG integration tests, cleans up. No VM, no Docker. Fast (~3s startup). Works on macOS too. Supports `PGDATA_CACHE` to skip `initdb` on repeated runs (M35).
  - **Ephemeral Redis** (`nix run .#ephemeral-redis`): starts a Redis server from nixpkgs for Watermill adapter testing. Sets `REDIS_URL`. Auto-cleanup.
  - **Ephemeral NATS** (`nix run .#ephemeral-nats`): starts a NATS server with JetStream from nixpkgs. Sets `NATS_URL`. Auto-cleanup.
  - **Ephemeral Dgraph** (`nix run .#ephemeral-dgraph`): starts Dgraph Zero + Alpha from nixpkgs for dgraphengine integration testing. Sets `DGRAPH_ADDR`. Health-check loop waits for Alpha readiness. Auto-cleanup. Run: `nix run .#ephemeral-dgraph -- go test -tags "goexperiment.jsonv2" -v ./metaengine/dgraphengine/...`.
  - **Cross-backend tests** (`nix run .#test-all-backends`): runs tests across ALL backends (SQLite, Pebble, bbolt, DuckDB, PG, MySQL) in one command. Use `--embedded-only` or `--external-only` to scope.
  - **PG strategy profiling** (`bash scripts/profile-pg-strategies.sh`): measures ephemeral vs testcontainers startup time. Ephemeral is 12x faster (1.2s vs 14.5s).
  ...
  - VM tests live in `nix/vm/postgres.nix` + `nix/vm/mysql.nix` and are wired as `checks` in flake.nix. The `nixos-vm-tests` CI job runs them. Additional VM tests: `duckdb-vm` (DuckDB columnar ops), `turso-vm` (libSQL server CRUD). Run: `nix build .#checks.x86_64-linux.duckdb-vm -L` / `turso-vm`.
...
- **Dedup helper patterns** (introduced 2026-07-27, updated 2026-08-09): `storage/memory` uses `withWriteLock(code, msg, fn)` methods (write-side) + `withReadLock[T](s, code, msg, fn)` top-level generic functions (read-side, Go has no generic methods) + `wrapClosed(err, code, msg)` for the `CheckClosed → WrapInfrastructure` nil-short-circuit pattern — each store defines its own pair because `CheckClosed` sentinels differ per store. Test helpers: `parallelTimeoutCtx(t, timeout)` in benchkit, `parallelViewStore(t)` in storage/view, variadic `NewTestRegistry(svc...)` in catalog. Engine test helpers: `mustNewPgEngine(t)` in pgengine, `mustNewDuckEngine(t)` in duckdbengine, `setupSeededAggTest(t)` in sqliteengine, `assertTxCommitSetup(...)` in enginetest, `saveOneCommand(...)` in `command/commandtest`. Production helpers: `metaengine.DeferClose(c Closer)` replaces `defer func() { _ = x.Close() }()` across all engine modules (47 production + 17 test sites), `stdQueryInit(...)` in duckdbengine for SQL query initialization, `system.drainAll(ctx)` for ordered shutdown draining. `benchkit.TitleCase` and `benchkit.Truncate` are shared exports used by `cmd/cqrs-bench`. `cmd/cqrs-lint/explain.go` uses `renderTable` + `writeTableRow` + `writeTableSeparator` + `columnWidths` for variable-column table rendering. The `.art-dupl-baseline.json` golden + `nix run .#check-duplication` gate enforce no-new-clones; run `art-dupl baseline . --threshold 3 --semantic` to update after an accepted consolidation. Coverage drift is checked by `nix run .#check-coverage` (`scripts/check-coverage.sh`).
- **`testModules` ↔ `lintModules` coupling in flake.nix** — the `testModules` list in `flake.nix` feeds BOTH `nix run .#test` and `nix run .#lint` (lint is `lintModules = testModules`). Adding a new module requires adding its path to `testModules` in `flake.nix` — otherwise it is silently never built, tested, or linted. The meta-test `TestEveryGoModDirIsInTestModules` in `cmd/api-stability/main_test.go` enforces this automatically by parsing `flake.nix` and asserting every `go.mod` directory appears in the list. Run `cd cmd/api-stability && GOWORK=off go test -tags "goexperiment.jsonv2" -run TestEveryGoModDirIsInTestModules .` to verify.
...
- **CBOR int→uint64 type drift (QUIC transport)** — CBOR encodes Go `int` as unsigned integers (major type 0); on decode into `any`, fxamacker/cbor produces `uint64`, not `int`. This silently breaks `val.(int)` type assertions on receivers. The QUIC transport's `decodeOp` in `quic/latency.go` runs `normalizeAny()` after CBOR decode to coerce `uint64`→`int` and `int64`→`int` recursively. Tests can use `gomega.Equal` (not `BeEquivalentTo`). Any future transport that uses CBOR to encode `any`-typed fields must apply the same normalization.
- **Dgraph 25.x `DeleteJson` requires explicit null predicates** — Dgraph 25.x does NOT delete all predicates when `DeleteJson` contains only `{"uid": "0x1"}`. The node remains fully intact. You MUST list each predicate as `null`: `{"uid": "0x1", "cqrs.map_collection": null, "cqrs.map_key": null, "cqrs.map_value": null}`. This is a behavior change from older Dgraph versions where bare `uid` deletion cleared all predicates. The `dgraphengine.MapDelete` function handles this via the upsert pattern with explicit null-predicate deletion. The `TestMapBackend` test verifies the delete-then-read cycle against a live Dgraph instance via `nix run .#ephemeral-dgraph`.

## Dependencies

| Category   | Packages                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Production | oklog/ulid/v2, go-branded-id, go-error-family, go-retry (middleware), go-idempotency (idempotency/), go-faster/yaml (catalog); go.opentelemetry.io/otel (otel, event, storage, middleware, prometheus); prometheus/client_golang (prometheus); golang.org/x/crypto (encryption); fxamacker/cbor/v2 (codec); maypok86/otter/v2 (kv, decider); failsafe-go/failsafe-go (middleware); modernc.org/sqlite (storage, stack/sqlite — pure-Go); github.com/jackc/pgx (storage, stack/postgres); github.com/go-sql-driver/mysql (stack/mysql — pure-Go); github.com/duckdb/duckdb-go (stack/duckdb — CGo, C++ engine); cockroachdb/pebble (storage/pebble); go.etcd.io/bbolt (storage/bbolt) |
