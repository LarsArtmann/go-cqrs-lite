...
Multi-module Go workspace (`go.work`) with 53 `go.mod` files — all wired into `go.work`. Breakdown: 34 library + 7 stack presets + 8 examples + 3 cmd + 1 integration. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
...
Layer 1: event/ (→id, codec, ro), command/ (→id, dispatcher, ro), query/ (→dispatcher, ro), scheduling/ (→id)
