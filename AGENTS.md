...
11. **Deletion as domain events** — Soft-delete via domain events (ADR-0114), not mutable metadata. Deletion is expressed as an event type (e.g. `user.deleted`). Stream status is `StatusActive`/`StatusDeleted` (detected via `WithDeleteTypes`). No Delete on Store.
...
- **modernc SQLite `file::memory:` is per-connection** — every pooled connection gets its own private, empty database. Any query overlapping another connection's lifetime (e.g. a timer scheduler polling while a tx holds another connection) lands on a schema-less DB → `no such table` flakes under parallel test load. Shared in-memory DBs must pin the pool to ONE connection (`storage.OpenSQLiteInMemory` does; `ConfigureSQLitePool`) or use a named shared-cache DSN. Regression test: `TestOpenSQLiteInMemory_SingleSharedDatabase`.
