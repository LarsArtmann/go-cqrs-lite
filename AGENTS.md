...
Multi-module Go workspace (`go.work`) with 59 `go.mod` files — all wired into `go.work`. Breakdown: 42 library + 8 stack presets + 3 examples + 5 cmd + 1 root workspace. Verify: `find . -name go.mod -not -path './vendor/*' | wc -l`:
