---
name: go-cqrs-lite
description: Build Go applications with go-cqrs-lite — the composable CQRS + event-sourcing library. NEW APPS MUST build on the `system` composition root (ADR-0123: `system.New(ctx, DomainConfig, DeploymentConfig)` auto-wires event store, bus, projections, deciders, queries, snapshots, timers, health, lifecycle) and the `metaengine` cost-based storage planner (`Store` + `Query[Q,R]` auto-projection across interchangeable engines: memory, sqlite, turso, pebble, bbolt, badger, dgraph, duckdb, postgres, mysql, bigtable, iroh CRDT (see `metaengine/*engine` for the full set). The low-level modules (event, command, decider, storage, stack, kv) are legacy/migration-only for new code. Use when a project imports any github.com/larsartmann/go-cqrs-lite/*/v4 module, OR the user asks how to build CQRS/event-sourcing systems in Go, dispatch commands/queries, build read models or projections, use event stores or buses, or work with any go-cqrs-lite module (event, command, query, decider, storage, stack, projection, schema, middleware, catalog, watermill, scheduling, deriver, graph, metaengine, system, projectionhost) — even when the library is NOT named explicitly (e.g. "set up event sourcing", "build a read model", "dispatch a command", "snapshot a stream", "replay events", "idempotent commands", "cost-based query planner", "operator-picked engines").
user-invocable: true
metadata:
  tags: cqrs, event-sourcing, go, composition-root, metaengine, storage-engine, projection, read-model, event-store, domain-driven-design
---
...
## REQUIRED: build on `system` + `metaengine`
...
> **v5 deprecation notice (ADR-0123):** all v1 read-model tiers below and the
