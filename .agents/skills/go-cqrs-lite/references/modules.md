...
| `storage`  | `storage/v4`        | `SQLEventStore`, `SQLSnapshotStore`, `SQLCheckpointStore`, `SQLCommandStore`, `SQLQueryStore`, `SQLKVStore`, **`SQLViewStore`** (column-mapped views). PG/SQLite/MySQL. `NewSQLiteBackend`/`NewSQLBackend`/`NewMySQLBackend` facade. `sql/` sub-package: `RunInTx`, `IsDuplicateKeyError`, `ScanSlice`. |
...
| `metaengine/pgengine`          | `metaengine/pgengine/v4`          | Postgres-backed metaengine `Engine` (JSONB + B-tree). `MapBackend`, `CounterBackend`, `ScanBackend`, `PushdownScan` (JSONB operators), `LayoutPlanner` (expression indexes). Pure Go (pgx). ADR-0087.                |
