...
// duration — MySQL is multi-writer like Postgres) for fact attribution:
// the finalize methods carry the token, not the owner, so the journal's
// attribution comes from the row itself.
func leaseHolder(ctx context.Context, q taskQuerier, id task.ID) (string, error) {
	var owner string

	err := q.QueryRowContext(ctx,
		`SELECT lease_owner FROM tasks WHERE id = ? FOR UPDATE`, id.String()).Scan(&owner)
...
// locking the row for the tx's duration (multi-writer discipline).
func readAttemptHolder(ctx context.Context, tx *sql.Tx, id task.ID) (int, int, string, error) {
	...
	err := tx.QueryRowContext(ctx,
		`SELECT attempts, max_attempts, lease_owner FROM tasks WHERE id = ? FOR UPDATE`, id.String()).
...
func (s *Store[T]) FailPermanent(
	...
	return s.withTx(ctx, func(tx *sql.Tx) error {
		...
		err := tx.QueryRowContext(ctx,
			`SELECT attempts, lease_owner FROM tasks WHERE id = ? FOR UPDATE`, id.String()).
			Scan(&attempts, &owner)
		if errors.Is(err, sql.ErrNoRows) {
			return queue.ErrNotFound
		}
