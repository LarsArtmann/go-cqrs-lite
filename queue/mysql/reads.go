...
func (s *Store[T]) scanTaskRow(r scanner) (task.Task[T], error) {
	...
	if deps != "" && deps != "[]" {
		if err := json.Unmarshal([]byte(deps), &t.Deps); err != nil {
			return task.Task[T]{}, fmt.Errorf("queue/mysql: unmarshal deps for %s: %w", id, err)
		}
	}

	decoded, err := s.codec.Decode(payload)
	if err != nil {
		return task.Task[T]{}, fmt.Errorf("queue/mysql: decode payload for %s: %w", id, err)
	}

	t.Payload = decoded

	//art-dupl:accept dialect twin of queue/sqlite+postgres Get decode tail; only the driver surface differs
	return t, nil
}
...
func listWhere(f queue.Filter) (string, []any) {
	...
	//art-dupl:accept dialect twin of queue/sqlite+postgres listWhere tail; placeholder spelling only
...
func (s *Store[T]) List(ctx context.Context, f queue.Filter) ([]task.Task[T], error) {
	...
	//art-dupl:accept dialect twin of queue/sqlite+postgres List scan loop
	return out, rows.Err()
}
...
// matches literally. Pair with ESCAPE '\\' in the SQL (MySQL shares the
