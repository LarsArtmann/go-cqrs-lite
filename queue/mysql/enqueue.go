...
// fact (idempotent enqueue; the UNIQUE KEY on the nullable dedup_key is
// the final arbiter — MySQL treats NULLs as distinct, so keyless tasks
// never collide).
func (s *Store[T]) Enqueue(ctx context.Context, n task.New[T]) (task.Task[T], error) {
	//art-dupl:accept dialect twin of queue/sqlite+postgres Enqueue flow; conformance pins idempotency
	n = n.Normalize()
	if n.Type == "" {
	...
	if n.DedupKey != "" {
		if existing, found, err := s.getTaskByDedupKey(ctx, n.DedupKey); err != nil {
			return task.Task[T]{}, fmt.Errorf("queue/mysql: enqueue dedup lookup: %w", err)
		} else if found {
			return existing, nil
		}
	}
	...
	if err := s.insertTask(ctx, &created, n.DedupKey, &suppressed); err != nil {
		return task.Task[T]{}, fmt.Errorf("queue/mysql: enqueue: %w", err)
	}
	...
	return s.withTx(ctx, func(tx *sql.Tx) error {
		if dedupKey != "" {
			...
			err := tx.QueryRowContext(ctx,
				`SELECT id FROM tasks WHERE dedup_key = ?`, dedupKey).Scan(&existingID)
			if err == nil {
		...
		if err := validateDeps(ctx, tx, t.Deps); err != nil {
			return err
		}
...
// insertTaskRow runs the tasks-table INSERT (dedup NULL when keyless).
...
func (s *Store[T]) getTaskByDedupKey(ctx context.Context, key string) (task.Task[T], bool, error) {
	//art-dupl:accept dialect twin of queue/sqlite getTaskByDedupKey; conformance pins dedup-key idempotency
	var id string
