...
// For memory backend, use the memory module: github.com/larsartmann/go-cqrs-lite/memory
...
func NewStoreFromConfig(cfg StoreConfig) (Store, error) {
	...
	return nil, fmt.Errorf("use the memory module for in-memory store; unknown event store backend: %q", cfg.Backend)
}
