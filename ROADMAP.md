...
v4.8.1 + the `/v4`-suffix guard in tag-release.sh), `cmd/cqrs-lint/v4.9.0`
(09-06: V007 `v5-removed-api-usage` + hardening), and the SUPERB adoption
...
`[Unreleased]` window carries the 2026-09-06..21 waves (see the Release
...
| [Unreleased]                 | —          | • **2026-09-21 (v4.9.0 release wave)**: 4-tag dependency-ordered train —
  `record/v4.6.0` (`record.DeferClose`, ADR-0144), `scheduling/sqlstore/v4.1.1`,
  `projectionhost/v4.5.1` (live/catch-up double-apply fix, found by the new
  examples CI leg), `system/v4.9.0` (fluent `Evolve(...).On(...)` chaining +
  fail-closed racy-save: `ErrRacySaveRefused`/`WithRacySave`/`ErrEventSaveNotAtomic`);
  every example adopted published pins (goal-shaped-app fluent `.On` + scenario
  Given/When/Then + snapshot demo; scheduler-otel-status consumer-pure), and the
  `#test-examples` flake app + `Examples Test` CI job landed — examples are
  tested, not just built.<br/>• **2026-09-21 (guard + trust-closure wave, four parallel sessions)**: verification-guard buildout — `.golangci.yml` sha256 hash-golden tripwire (caught incident #11 live: a 111-file auto-commit had downgraded go + resurrected jsonv2 + deleted the depguard allow-list; repaired from last-good), `can-run-composed-gate --wait-loop`, `preflight-composed.sh` (caught real templ drift on first run), in-`#verify` load guard + machine-level verify flock + `check-go-version` contract gate, CI coverage-gate rebuilt on a real core-tier floor (was a vacuous empty-`go list` loop) + README truth gates onto the push leg + CI autoretry workflow; benchmark-gate trust floor (T18b) — `write_p99_ns` demoted from the noise headline (estimator-variance evidence), `--save` refuses on noise-gate failure (`--force-save` override), `KNOWN_UNSTABLE` suppression with expiry, per-suite `benchtime::count` + CI parity, `quiet-window-run`/`nightly-bench` flake apps + SystemNix nightly timer, autonomous widening/campaign chain armed; queue M4 tail (MySQL deadlock backoff+jitter, pool options, `testutil/mysqltestcontainer` = module #96, PG `-race -count=2` caught + fixed a real shared-DB parallel-migrate flaw) + PapDashboard ADOPT verdict; benchkit CLI polish tail 16/17 slices (Min rendering, `--strict` noise gate, reservoir size, `RunSuiteRepeated`, rename guards + the fixture-litter fix); md-go-validator docs gate (103-error baseline, archived-only policy, `#check-md-go` in verify+CI); docs-truth wave (doc-check to true zero warnings, FEATURES census to all 96 modules, per-day archived-waves index, status live-index); three design one-pagers (engine single-writer lease / AggregateOn seam / Scan-default v5 survey); T23 skill-maintenance pass + benchkit progress-goroutine race fix; 10th docs-health pass (11 reports archived, TODO rebuilt)<br/>• **2026-09-20 (verify-green + publish-verification arc)**: composed `#verify` GREEN — the S03 record, all 19 phases (first composed green since 09-09), publish verified end-to-end (92 GitHub Releases, proxy `@latest`, `pin-sweep --check --remote`, retracts, audit-baseline 0 NEW, smoke 91/92), camelCase planned-column pushdown silent-empty fix (`metaengine.ExtractFields` matches json tag OR Go field name), ctx-scoped transactions ported to the pg/mysql/duckdb engines, `system.GracefulClose` pre-cancelled-context determinism fix, README deprecation-honesty gates (`check-readme-links.sh`/`check-readme-deprecated.sh`, nightly-wired, baseline 0) + the ADR-0123 README banner sweep (deprecated citations 37→0), `# Experimental` doc.go stamps on all 17 metaengine-family modules, module-map census to all 96 go.mods, coverage re-baseline, `example/taskmanager v0.2.1`<br/>• **2026-09-17..19 (substrate + rigor arc)**: ADR-0141 temporal versioned cells (memory/sqlite/bigtable engines, AsOf input routing, `temporal-asof`), ADR-0142 universal storage substrate (`DueClaimer`/`DedupStore`/`FactSink` capabilities, claimkit one database/sql runtime, queue engines as metaengine drivers incl. new `queue/mysql`, `scheduling/engine` TimerStore facade, capability audit + Doctor rendering, reset ladder incl. claimkit collections), queue M4 (ADR-0134 claim tokens, enqueue dep validation, `FactTx`/`Watermarks`), ADR-0143 journal-survives-reset (root-caused the days-old replay-starvation flake), Go 1.27.1 cutover + jsonv2 graduation sweep (95 modules), repo-wide lint debt → 0 (exhaustruct panic class killed), benchkit statistical-rigor tail (variation columns, per-metric CI gating + load/noise gates, SDK polish), `example/goal-shaped-app` ("The Goal in 5 minutes"), CI hardening (canonical pre-commit hook, depguard self-heal + golden, nightly-gates cron, magic-nix-cache removal from all 24 CI jobs)<br/>• **2026-09-13..16 (six-session arc)**: command-side depth W1–W3 (`decider.ExecuteCommandRef` + `CausedCommand` causation stamping, `command.AsRecordPersisted` full-fidelity bridge, lifecycle upcast composition pin), event-query-model truth reconciliation (`metaengine.Store.StreamCollection` streaming export + `CommandsByActor` per-actor projection + semantics pins), `claiming/` extraction (byte-identical SKIP LOCKED claims; timers delegate) and the **queue family** (`queue` contract + `queue/conformance` suite + `queue/sqlite` single-writer + `queue/postgres` SKIP LOCKED engines, all conformance-green incl. `-race`), T17 rejection partition (`command.rejected` + `RejectionLog`; DLQ/FailureLog unchanged), skill-docs audit wave (14+9 defects fixed; `cmd/doc-check` gained TOC-anchor/§-crossref/call-arity gates + the recipes compile harness 77/77 classified), vector search on EVERY engine (brute-force + DuckDB/libSQL pushdown, honest O(N) profiles), benchkit statistical rigor (`RunRepeated`/`MetricVariation`/exact P100/real benchstat samples/load provenance), CI kill-switches (`modsums` job + `TestEveryModuleGoSumIsTidy`, `integration-tag-lint`, permanent aggregate-tripwire fixture, staged-aware pre-commit gates, `pin-sweep --dry-run/--remote`, `calibration-drift --baseline`), `metaengine.Infer` deprecated (v5), ADR-0139 skeleton (v5 encryption-at-rest), fifth `.golangci.yml` config-corruption recovery<br/>• **2026-09-11 (publish/reset/v5-train waves, two parallel sessions)**: `EngineResetter` on ALL 12 engines (ADR-0136 ladder complete; `CanReset` in `GetEngineStats` + Doctor `--- Reset ---` section), fold-write failover + `Store.CatchUpEngine` (ADR-0137 completion), `metaengine.ApplyEncodedRecord` + the 13-entry-point record-context conformance sweep (encoded-apply side door closed), dgraph `WithContentionObserver` + honest-loud live-test construction, turso `file:` DSN fix + three-defect IVM repro suite (`-tags ivmrepro`) + single-sourced version-citation gate + flip runbook, error-taxonomy drift gate (5 modules, bidirectional), cqrs-upgrade strict-gate hardening + example v5-clean meta-test, C040 catalog parity (E018 provider contract), goleak gates (metaengine/projectionhost), `[Unreleased]`-position tripwire, release-tooling hardening (batch-release bar + fixture suites in CI, go-must private-dep fix), 350-line baseline+ratchet gate (green), coverage-gate loud-failure fix<br/>• **2026-09-09..11**: Cordis execution — one-call reset (`EngineResetter`/`ResetResult`, ADR-0136), coeffect gate (`DomainConfig.Events`), cqrs-lint E018 + F091 Tier-3 + T13–T19 audits, ADR-0137 engine deactivation, scenario equivalence tooling; issue-#21 watermill typed-causation wire protocol (untagged); issue-#20 closeout (cqrs-bench v0.1.1 stub, cqrs-lint v4.10.1 retract-carrier, badgerengine v4.2.1 data-loss retracts, tag-release `--audit`/`--smoke`); dgraph `-shuffle=on` rollout + contention retry fix; ClaimMetrics `Metrics()` snapshot; Demote record-context fix; benchkit expired-ctx loud failure<br/>• **2026-09-08**: errorfamily codes renamed to stream vocabulary (17 codes, 9 modules, dashboards note); cqrs-lint analyzer hardening (deterministic store detection, engine coverage, doctor JSON golden); bbolt read-only opens + wire-format golden (coordinated 15-module re-tag)<br/>• **2026-09-07 SUPERB adoption wave**: `cmd/cqrs-upgrade/v4.0.0` (consumer pin-bump + V007 CLI), `otel/v4.4.0` (ForceFlush-before-Shutdown + `WithSpanProcessor`), benchkit system harness (`FactoryFromSystem`/`AdaptSystem`), go-appkit/cqrs v0.5.0 on `system.New` (v5 cliff defused for the on-ramp), skill quickstart leads with `system`, Turso materialized views (ADR-0135) + embedded encryption (`WithEncryption`), metaengine correctness batch (capability gaps through Plan, keycodec, FromDB harness), cqrs-lint F089/F090/F091-Tier1<br/>• **2026-09-06**: eight waves — cqrs-lint severity/confidence contract + `rules --json` + suppression/fix/lintutil hardening + self-healing formatter guard; metaengine correctness (plan-time capability partition, record-context advisory, MariaDB SKIP LOCKED claiming, planned-table parity, dgraph per-row recalibration); iroh graph WriteOp convergence; catalog/encryption (key-management helpers, envelope v2, rotation write-back, `check-csp`/`check-eventcatalog`); honest snapshot wire tags (T18) + `MigrateSnapshotColumnsToStream`; watermill CatchUp hardening + events-DDL re-exports; tooling batch (badger restart data-loss fix, `SortPaginate[T]`, doc-check block-scoped resolver, pin-sweep, exhaustruct_v5)<br/>• **2026-09-05**: V007 `v5-removed-api-usage` (204 rules) + getting-started modernized onto `system.New`<br/>• **2026-09-03**: master-CI repair wave 2 (FlakeHub, module matrix discovery, go.work externals)<br/>• **2026-08-30/31**: D3 planned-table pushdown train (filters/sort/keyset, MapScan/MapUpdate, EXPLAIN proofs, cross-engine parity matrix), `LayoutPlanEvolver`, `BackfillPlannedCollection`, `EffectiveDurability`, `RenewLease` + claim metrics<br/>• **v5 pre-cut**: 73 deprecation markers (stack tiers, view/relational, tombstone API, transport) landed 2026-08-17<br/>• **Durability tiers**: pebble/postgres/bbolt/badger map Strict/Normal/Relaxed (08-17/18)<br/>• **Vector/graph**: binary float32 vector payloads (~31-35x search), depth-1 graph short-circuit, filtered k-NN, GraphRemoveEdge (08-16/17) |
...
sections — the 2026-09-07/08 correctness batch (ApplyBatch Record handling,
record-aware-cache invalidation, Doctor observations, MySQL claiming,
dgraph calibration, planner polish, keycodec extraction) SHIPPED in full,
and the 2026-09-11 waves closed the matview safety tail (Doctor-section
pin, `-tags ivmrepro` suite, citation gate, flip runbook) plus the reset
ladder and write failover. What remains: the iroh standalone-pin repair,
the skill-reference propagation wave, quiet-window calibration re-runs,
and the `CatchUpEngine` snapshot-race fast-follow.
...
**Remaining (short-term):** See [TODO_LIST.md](TODO_LIST.md) → Metaengine
sections — the correctness/verification tail above is closed (shipped
2026-09-07/08: layout calibration, capability conformance, DuckDB aggregation
pushdown, seq-carrying journal reads, calibration regression checking).
Open remainder: iroh standalone-pin repair + the skill-ref propagation wave.
...
- **Vector search at scale — int8 quantization + ANN indexing** — adopt when a
  ...
  `docs/planning/archived/2026-08-16_VECTOR-SEARCH-AT-SCALE-SPIKE.md`); first candidate: a `sqliteengine` vector32() option (go-graph-rag feedback #1)
...
- **Quarantine-aware replan** — after a long quarantine, replan onto the
  failover engine instead of falling back on reactivation (cost-model
  honesty). — 05-40 §f34
- **`CatchUpEngine` replay-into-a-NEW-engine** — zero-downtime engine swap
  riding the same reset+replay primitive. — 05-40 §f35
- **EventLog growth bounds / rotation policy** — full-journal replay cost for
  long-lived catch-up-enabled stores. — 05-40 §f37
- **Span-name drift gate** — the `{component}.{action}` contract
  (`docs/SPAN_NAMING.md`) has no mechanical gate; same script pattern as the
  error-taxonomy gate. — 05-26 §f35
- **`cqrs-upgrade --workspace --strict` as a CI job on this repo** —
  self-hosted dogfood of the v5-readiness gate. — 05-26 §f36
- **Gate the codec-defaults table in AGENTS.md against source** — third
  consumer of the drift-gate script pattern. — 05-26 §f37
- **Canonical-fact gate pattern** — when a third copy of a load-bearing
  number appears in docs (e.g. the turso "~27k" wall figure), demand a
  canonical constant + gate like `check-turso-version`. — 05-21 §e4/§f13
- **Golden-profile harness for typed lint gates** — auto-regen + review flow
  per rule; would have de-risked the S001/D016/A017 classes. — 05-12 §f25
- **Move example apps out of the workspace** — workspace unions are the root
  hazard class behind the go-must incident; examples as plain checkouts
  cannot poison the union. — 05-34 §f49
- **Owner decision doc: public library + private helper repos** — a
  structural contradiction; pick the end state (publicize, vendor, or
  internal-only consumers). — 05-34 §f50
- **Claims-ledger convention for parallel agent sessions** — claim-before-
  write on TODO items (the two-session collision cost ~60 min of duplicate
  design on 2026-09-11). — 05-40 §f38, 05-51 §e1
- **Richer graph edges for GraphRAG consumers (go-graph-rag feedback #7)** —
  capability-interface step beyond `graphadapter`'s edge model (weighted/typed
  edges, edge payloads). Demand signal: the 2026-09-15 consumer evaluation.
  — source: go-graph-rag feedback (archived 2026-09-22)
- **Consumer-DX batch (go-graph-rag feedback #8)** — fold-signature error
  tables embedded in classifier errors; publish `references/modules.md` via
  catalog/docserver; a plan-diff CI recipe for consumers; Doctor/ExplainPlan
  warn on cross-engine fold spans. — source: same evaluation
- **Doc-comment snippet checker** — the `Evolve` Level-1 doc-comment lie lived
  months because code comments aren't fence-gated (`#check-md-go` covers only
  Markdown). — source: 2026-09-21 23-24 followups §f29
- **Example-pins-as-code** — one source generating all six examples' pin sets
  per wave (kills manual bump drift); consider a `pin-sweep`-style
  example-freshness flake app + one-command `go get`-all-examples refresh.
  — source: 2026-09-21 23-24 closeout §f37/§f48
- **Session-claim convention for shared living docs** — TODO_LIST/CHANGELOG
  co-edits by concurrent sessions survived by luck during the v4.9.0 wave;
  a claim-before-write note (or worktree-per-doc-session) de-tangles.
  — source: closeout §e7/§f39
- **Tag-message convention** — link the CHANGELOG wave section in annotated
  tag bodies (v4.9.0-wave tags are one-liners). — source: closeout §f44
- **Release-flow `--changelog-check`** — `tag-release.sh` warns/blocks when
  `[Unreleased]` still cites the module being tagged. — source: closeout §e3/§f38
- **Harvest tag-tree evidence rule** — docs-health harvest notes must cite
  which symbols were checked in which tag tree (the "unblocked NOW by
  system v4.8.0" fiction cost a research round). — source: closeout §e4/§f41
- **Dev-replace strategy: go.work-only resolution** — prefer workspace-USE
  over per-module sibling replaces where possible; four modules carried
  "unpublished symbol" replaces that comment-rotted. — source: closeout §f36
- **Example-canary systematization** — one deliberately non-idempotent fold
  test per delivery seam (the getting-started counter caught a real
  projectionhost bug); keep as regression tripwires. — source: followups §f44
- **`#verify` + `#test-examples` merge question** — a verify run that skips
  examples can green a tree that breaks them (CI covers it; local blind
  spot). — source: closeout §f49
...
11. ~~**Skip-vs-fail for live conformance construction** (2026-09-11): when
    ...
    dgraph. — source: 02-16 §g3~~ **ANSWERED 2026-09-11: honesty-first** —
    `newDgraphEngineOrSkip` skips ONLY on server-unreachable errors and
...
16. **Push cadence (2026-09-22; asked by four sessions in two days):**
    ...
    `Examples Test` job, guard-wave legs) is gated on it. Batch-push on a
    cadence, or push immediately after each wave? (Also decides whether the
