...
- [ ] **benchkit noise gate: `write_p99_ns` chronically exceeds the 10% CoV threshold on this host (sqlite/dev profile)** — observed 2026-09-20 across 4 gate runs (CoV 11.5% → 20.6% → 22.5%) including a deep-quiet window (load 2.89/4.23) and at `--noise-repeat 7`; the other 3 headline metrics stay stable. An extreme-quantile estimator (~100-iter runs) has intrinsic cross-run variance, so a clean baseline save is a coin flip around the threshold. Compounding: `BenchmarkMatViewRead/agg=MIN/scale=1k/matview` is bimodal within runs (samples 7.9-15µs), so the 25% median-of-5 compare flags a different matview variant on different runs (MIN twice, SUM_VIA_GROUPED/baseline once) — gate-level flake, NOT perf evidence (the save-run compare was 0-regressions). Decide: p99-aware noise threshold (or p50+max headline pair), wider benchtime/count for the matview gate entries, or a `//art-dupl`-style accept annotation. Related: "Calibration provenance protocol" row (c)/(d) and the W2 quiet-window tooling row.
