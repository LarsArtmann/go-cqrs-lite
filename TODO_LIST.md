...
- [ ] **Turso MVCC concurrent-write support** [BLOCKED UPSTREAM] — The Turso Database engine has an experimental MVCC mode (`PRAGMA journal_mode=mvcc` + `BEGIN CONCURRENT`) that enables concurrent writes with row-level conflict detection. Once stable, it would allow raising `MaxOpenConns` above 1 for real write parallelism. Currently blocked: (1) MVCC is still experimental with known limitations (no `CREATE INDEX`, inefficient row versioning, no async I/O, no `WITHOUT ROWID` writes), (2) our `RunInTx` uses standard `BEGIN`, not `BEGIN CONCURRENT`, (3) optimistic concurrency requires application-level conflict-retry logic. When unblocked: add `ConfigureTursoMVCCPool`, a `BeginConcurrent` transaction option, and conflict-retry middleware. See `ConfigureTursoPool` in `storage/sqlite_helpers.go` for current WAL-mode rationale.
