...
- [x] ~~Quiet-window tooling promotion (W2 row)~~ **DONE 2026-09-21 (owner ruled promote-now):** `scripts/quiet-window-run.sh` — waits load1/load5 < `--ceiling` (default 5), runs the wrapped command with `--attempts`/`--no-retry` bounded retries, `--deadline` (default 6h), `--log`; `--self-test` 6/6 green (quiet+pass, never-quiet deadline, retry-then-pass, always-fail exhausted, usage); wired as flake app `nix run .#quiet-window-run -- CMD` (mirror of load-sweep). Open halves: nightly-gates wiring (follow-up), host benchmark-ceiling policy (owner UNDECIDED 2026-09-21 — ceilings stay strict <5 until ruled).
