...
// TestExporter_OperationMessagesBelongToChannel is a regression test for the
// AsyncAPI 3.0 parser rule asyncapi3-operation-messages-from-referred-channel.
//
...
func TestExporter_OperationMessagesBelongToChannel(t *testing.T) {
	t.Parallel()
	...
	for opName, op := range doc.Operations {
		channelRef := op.Channel.Ref
		if channelRef == "" {
			t.Errorf("operation %q has no channel ref", opName)
			continue
		}

		expectedPrefix := channelRef + "/messages"

		for i, msg := range op.Messages {
			if msg.Ref == "" {
				t.Errorf("operation %q message[%d] has no $ref", opName, i)
				continue
			}

			if !strings.HasPrefix(msg.Ref, expectedPrefix) {
				t.Errorf(
					"operation %q message[%d] $ref = %q; must start with %q "+
						"(AsyncAPI rule: operation messages must belong to the channel)",
					opName, i, msg.Ref, expectedPrefix,
				)
			}
		}
	}
...
// TestExporter_ChannelTitlesUseSingularKind validates that channel titles use
// the proper singular form ("query", not "querie") and never produce leading-
// space garbage when msg.Name is empty.
func TestExporter_ChannelTitlesUseSingularKind(t *testing.T) {
	t.Parallel()

	reg := cattest.NewTestRegistry()
	cattest.AddServiceWithQuery(
		t, reg, catalog.ServiceID("svc"), catalog.MessageID("GetItem"),
		"GetItem", "1.0.0", "Get item",
	)

	cat := cattest.Build(t, reg)
	doc := NewExporter("Test", "1.0.0").Export(cat)

	ch, ok := doc.Channels["queries.GetItem"]
	if !ok {
		t.Fatal("missing queries.GetItem channel")
	}

	if strings.Contains(ch.Title, "querie") {
		t.Errorf("channel title contains typo 'querie': %q", ch.Title)
	}

	if !strings.Contains(ch.Title, "query") {
		t.Errorf("channel title should contain singular 'query': %q", ch.Title)
	}
}

// TestExporter_EmptyNameFallback validates that when msg.Name is empty (common
// for REST endpoint wrappers using struct{} generics), titles and operation
// names fall back to the message ID instead of producing empty/garbage strings.
func TestExporter_EmptyNameFallback(t *testing.T) {
	t.Parallel()

	reg := cattest.NewTestRegistry(
		catalog.Service{ID: "svc", Name: "Svc", Version: "1.0.0"},
	)
	reg.AddCommand("svc", catalog.Message{
		Kind:    catalog.CommandMessage,
		ID:      catalog.MessageID("rest.items.list"),
		Name:    "", // intentionally empty — mirrors REST endpoint wrappers
		Version: "1.0.0",
	})
	...
	ch, ok := doc.Channels["commands.rest.items.list"]
	if !ok {
		t.Fatal("missing commands.rest.items.list channel")
	}

	if strings.TrimSpace(ch.Title) == "" || strings.HasPrefix(ch.Title, " ") {
		t.Errorf("channel title should not be empty or start with space: %q", ch.Title)
	}

	if !strings.Contains(ch.Title, "rest.items.list") {
		t.Errorf("channel title should fall back to message ID: %q", ch.Title)
	}

	for _, op := range doc.Operations {
		if strings.TrimSpace(op.Title) == "" || strings.HasSuffix(op.Title, " ") {
			t.Errorf("operation title should not be empty or end with space: %q", op.Title)
		}
	}
}
