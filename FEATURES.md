...
| ClaimingTimerStore  | `scheduling/sqlstore.ClaimingTimerStore` — lease-based claiming for multi-instance dispatch: PG CTE `FOR UPDATE SKIP LOCKED` + `UPDATE..RETURNING`, SQLite single-writer; MySQL rejected loudly (`ErrClaimingUnsupported`); live two-claimer contention test | ✅     |
