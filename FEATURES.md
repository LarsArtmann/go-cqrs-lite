...
| One-call reset (ADR-0136)       | `Store.Reset(ctx)` + `EngineResetter` interface + `ResetResult` (with `Partial()`): wipe derived state and replay from zero. `metaengine/projectionadapter` implements `projectionhost.Resettable`; `WithKeepStaleState` opts out of the warn-first partial-reset guard. Memory engine fully resettable; persistent engines are follow-ups (warns loudly per ADR-0136's invertibility ladder)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 🧪     |
| Health-driven deactivation (ADR-0137) | Consecutive errorfamily Infrastructure/Transient failures quarantine an engine (`SetEngineFailureThreshold`, default 3): reads reroute via `effectiveQueryLocked` (plan untouched), the engine is never re-planned onto, `StartAutoReprobe` reactivates on a successful `Prober` probe, `ReactivateEngine` for manual recovery. Surfaced via `EngineStats.Health` + the Doctor `--- Engine Health ---` section. Writes to a quarantined engine fail loudly (fold-write failover is future work) | 🧪     |
