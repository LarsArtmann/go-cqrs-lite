...
| Feature                               | Detail                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Status |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| Unified `On[E]()` fold                | Reflection-based handler classification: 12 fold types (insert, update, set, count, edge, remove, skip, etc.). Sealed interface eliminates the 11-field `any` god-struct (ADR-0081)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 🧪     |
...
| BigTable engine                       | `metaengine/bigtableengine` — dep-isolated Google Cloud BigTable backend (ADR-0141 flagship): every map value a native timestamped cell (as-of reads, history, tombstones are BigTable primitives; retention = column-family GC policy), native ReadModifyWrite counters, self-registering `"bigtable"` driver. bttest-validated only — uncalibrated priors, no real-GCP run yet                                                                                                                                                                                                                                                                                                                                                                                                                                        | 🧪     |
...
| Native temporal versioned cells       | ADR-0141 contract on 3 engines: Memory (`NewMemoryEngineWithVersioning`, sorted chains + `WithRetention`), SQLite (`sqliteengine.WithCellVersioning`, `meta_cell_versions` table), BigTable (native cells, GC-policy retention). `VersionedWriter`/`VersionedUpdater`/`CellHistoryReader` capabilities, event-time fold stamping via `CellTimestamp`, AsOf input-field routing to `ExecuteAsOf` (+ `temporal-asof` planner warning on non-versioned engines), `adttest.AssertTemporalConformance` pins the contract cross-engine. Same-ms writes collapse LWW; tombstone = empty-value cell; out-of-order stamps legal                                                                                                                                                                                                  | 🧪     |
...
| Cost calibration                      | `ReadCosts` per read pattern (point-lookup/filtered-scan/aggregate/scan) on all 8 persistent engines (pg, mysql, sqlite, duckdb, dgraph, badger, bbolt, pebble) from calibration benches. `NsPerAggregate` = CounterGet per-row cost on every engine (ADR-0133, G1 closed 2026-09-01: pg 250 / mysql 320 / duckdb 420 / dgraph 2700 ns/row, all live-measured); e.g. sqlite point lookup ~3.1µs vs scan ~1.24µs/row. Dgraph scan/filtered-scan re-shipped as per-ROW constants (2_200 ns/row from scaled benches 2026-09-06 — the per-RPC fixed cost belongs to NetworkRTT, never a per-row field); constants are single-sourced from each engine's `Profile().ReadCosts` via `CALIB_DUMP=1` + `scripts/calibration-drift.sh`                                                                                           | 🧪     |
...
| ADT test harness                      | `adttest.RunMatrix` — cross-engine parity tests for all 10 ADTs. 150 specs run identical Apply → ExecuteTyped on Memory + SQLite. Reflect-based capability auto-detect + property-based (`rapid`) sequencing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 🧪     |
...
| Planned tables (pg+mysql)             | `ApplyLayoutPlan` (LayoutPlanApplier) materializes per-collection extracted-column tables; MapSet/Get/Delete, `MapUpdate` (new on both engines), `PushdownMapScan`, and `MapScan` route through the planned table; mis-typed filter/cursor values fail as `ErrPlannedColumnTypeMismatch` (Rejection) at query-build time; counters/graph/aggregates stay on meta_map (documented routing decision)                                                                                                                                                                                                                                                                                                                                                                                                                      | 🧪     |
...
| Aggregate pushdown (DuckDB)           | `AggregateReader`/`GroupedAggregateReader`/`MultiAggregateReader`/`MultiGroupedAggregateReader`/`ExplainableAggregate` — SQL COUNT/SUM/MIN/MAX/AVG pushdown. GROUP BY 4.4x faster, MultiAggregate 2.1x faster at 100K rows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 🧪     |
| Aggregate pushdown (SQLite)           | 4 aggregate interfaces on json_extract + planned-table paths. 10 planned-table subtests + 5 empty-collection edge cases                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🧪     |
| Aggregate pushdown (Postgres)         | All 5 aggregate interfaces + ExplainableScan. JSONB operator-based aggregation pushdown                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🧪     |
| TypedReader aggregate methods         | `GroupedCount`/`GroupedSum`/`GroupedMin`/`GroupedMax`/`GroupedAvg`, `MultiAggregate`, `MultiGroupedAggregate` — consumer-facing typed aggregate queries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🧪     |
| Cross-engine aggregate parity         | DuckDB vs SQLite aggregate parity tests + filter-based cross-engine parity tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 🧪     |
| DeferClose helper                     | `metaengine.DeferClose(c Closer)` — replaces `defer func() { _ = X.Close() }()` boilerplate across 47 production + 17 test sites                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 🧪     |
| Error sentinels                       | Exported `ErrNotFound`, `ErrAmbiguousKey`, `ErrUnsupportedADT`, `ErrLayoutConflict` wired into execution paths                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 🧪     |
| `OnTyped(eventType, ...)`             | Bind a fold to an explicit CQRS event-type string (decouples from the Go struct name). All enum families (ADT, Layout, FilterOp, etc.) have `Valid()` + registries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 🧪     |
| Vector ADT                            | k-NN similarity search (cosine/euclidean/dot) on EVERY engine (2026-09-15; LIVE-VERIFIED per engine 2026-09-16 incl. dimension lock + restart persistence + path reporting — ADR-0140, docs/benchmarks/2026-09-16_vector-search-paths.md): Memory/PG/MySQL/sqlite+Turso/Pebble/bbolt/badger/Dgraph brute-force, DuckDB + libSQL SQL-side pushdown, all with `VectorFilterBackend` + `VectorCounter`; ANN indexes remain trigger-gated (ROADMAP). ADR-0085                                                                                                                                                                                                                                                                                                                                                               | 🧪     |
...
| Iroh engine (Level 2)                 | `metaengine/irohengine` — `Replicated(localEngine, ...)` adds CRDT convergence via pluggable `Transport`. CRDT-safe: MapSet/MapDelete (LWW), SetAdd (OR-Set), CounterIncrement (PN-Counter), MultiAdd, LogAppend, GraphAddEdge/GraphRemoveEdge (per-edge LWW). Non-CRDT ops (MapUpdate, vector/search/spatial inserts) stay local                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 🧪     |
...
| `dgraphengine/`                       | Dgraph distributed graph backend via `dgo` gRPC + DQL. Map, Set, Counter, Graph, Search, Multimap, Log backends. DQL injection-safe (`QueryWithVars`). HealthCheck implemented (gRPC connection check). Calibrated from live benchmarks. ADR-0119. Contention-hardened: `retryOnContention` (6 attempts, jittered backoff) on every backend op, observable via `WithContentionObserver`; `ResetEngine` deletes engine-typed nodes + `cqrs.edge.*` predicates                                                                                                                                                                                                                                                                                                                                                            | 🧪     |
...
| Planner-time fold inference           | `metaengine.Infer(samples...)` inspects event/result types and generates folds automatically (composite keys, filter operators, sort fields, named events). `Override()` replaces a specific inferred fold. `OnRecord`/`OnRecordTyped` are the default constructors (ADR-0116 Layers 1-2). Infer + `InferFromNamedEvents` are **Deprecated: removal at v5** — production steer is explicit folds                                                                                                                                                                                                                                                                                                                                                                                                                        | 🧪     |
...
| `bboltengine/`                        | Bbolt LSM engine module. Self-registering driver. VectorInsert/VectorSearch via `keycodec` layouts. Source-of-truth persistence/restart/disk/calibration tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 🧪     |
| `mysqlengine/`                        | MySQL/MariaDB metaengine backend. Self-registering driver. Pure Go (`go-sql-driver/mysql`). `Prober` + `TransactMeasurer` wired. MariaDB dialect detection (Error-1064-safe pushdown, numeric-safe sorts). Native GraphBackend via `WITH RECURSIVE` (CTE-probed, iterative BFS fallback)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 🧪     |
| `tursoengine/`                        | Turso Database engine (embedded; the `turso` `database/sql` driver has no remote client). Self-registering driver. Remote DSN detection + `PingContext` prober. Materialized-view acceleration + embedded page encryption (rows below)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 🧪     |
| Materialized views (ADR-0135)         | `MaterializedViewSpec` (collection/fn/column/groupBy) declared via `sqliteengine.WithMaterializedViews` / `tursoengine.WithMaterializedViews` (auto `experimental=views` DSN) / `system` EngineConfig `materialized_views`: construction-time DDL, serving rewrite for scalar + grouped aggregates (SUM/COUNT/MIN/MAX/AVG), `Doctor` section with live row counts, `ExplainAggregateQuery` shows the view SQL. **Upstream caveat**: grouped views return silently wrong SUMs beyond one transaction's rows on tursogo ≤ v0.8.0-pre.10 (Doctor WARN + docs; scalar views exact) — see `docs/research/2026-09-07_turso-go-ivm-commit-failure-issue-draft.md`                                                                                                                                                              | 🧪     |
| Turso embedded encryption             | `tursoengine.WithEncryption(cipher, hexKey)` + typed `Cipher` constants (aegis256/aegis128l ±x2/x4, aes128gcm/aes256gcm) — opt-in experimental page-level AEAD for embedded Turso databases; wrong keys fail explicitly (`Decryption failed for page=N`); keys NEVER appear in errors or redacted DSNs. Direct `tursoengine.New` only (no `DriverConfig` slot yet — v5 `DriverConfig.Encryption` is the plan)                                                                                                                                                                                                                                                                                                                                                                                                           | 🧪     |
...
| Command lifecycle as events           | `commandlifecycle/` events (6 types incl. `command.rejected` with errorfamily-based partition — Rejection/Conflict families emit `rejected`, fold into `RejectionLog`, never DLQ/FailureLog; shipped 2026-09-15), `Recorder` (OCC version tracking, restart-safe), middleware pair, DLQ/retry/failure-log/processing-time/per-actor (`CommandsByActor`) projections, `system.WithCommandLifecycle()` wiring (ADR-0117)                                                                                                                                                                                                                                                                                                                                                                                                  | 🧪     |
...
| One-call reset (ADR-0136)             | `Store.Reset(ctx)` + `EngineResetter` interface + `ResetResult` (with `Partial()`): wipe derived state and replay from zero. ALL 12 first-party engines implement `EngineResetter` (2026-09-11 ladder complete: memory, sqlite, pg, mysql, duckdb, pebble, bbolt, badger, dgraph, iroh; turso by delegation) — layouts/schemas survive (post-Plan state), deletes are scoped to engine-owned storage, clears are atomic where the store allows, resets are idempotent, and journal sequences stay monotonic so pre-reset resumption tokens never skip replayed entries. `metaengine/projectionadapter` implements `projectionhost.Resettable`; `WithKeepStaleState` opts out of the warn-first partial-reset guard. `EngineStats.CanReset` + the Doctor `--- Reset ---` section surface capability BEFORE calling Reset | 🧪     |
| Fold-write failover + catch-up        | ADR-0137 completion (2026-09-11): folds reroute around a quarantined engine (same capability-aware partition rule as reads; execution-scoped, plan untouched); `Store.CatchUpEngine` resets the quarantined engine via `EngineResetter` and replays the attached EventLog into exactly its queries, lifting the quarantine only after a clean rebuild (`ErrCatchUpUnsupported` fallback); `StartAutoReprobe` prefers catch-up. Known fast-follow: the once-taken Events snapshot races concurrent applies during replay (TODO_LIST 🔥)                                                                                                                                                                                                                                                                                  | 🧪     |
| Encoded-apply record context          | `ApplyEncoded` + the new `ApplyEncodedRecord` flow through the same `applyWithRecord` funnel as every other dispatch path (metered, hooked, EventLog-recorded, advisory-counted, replicated) — encoded applies replay identically everywhere; `TestFoldDispatch_RecordContextConformance` pins record context + advisory counting across all 13 entry points                                                                                                                                                                                                                                                                                                                                                                                                                                                            | 🧪     |
| Health-driven deactivation (ADR-0137) | Consecutive errorfamily Infrastructure/Transient failures quarantine an engine (`SetEngineFailureThreshold`, default 3): reads AND folds reroute (execution-scoped, plan untouched), the engine is never re-planned onto, `StartAutoReprobe` reactivates via catch-up (plain-reactivation fallback), `ReactivateEngine` for manual recovery (documented stale-read caveat under write failover). Surfaced via `EngineStats.Health` + the Doctor `--- Engine Health ---` section                                                                                                                                                                                                                                                                                                                                         | 🧪     |

**Coverage:** ~80% (`go test -cover`). 174 BDD specs
...
| Coeffect validation gate        | Opt-in `DomainConfig.Events` declares the journal universe; a projection subscribing to an undeclared type hard-errors at `New` (`ErrDanglingEventSubscription`, fires before engine creation); a declared type nothing consumes is a `coeffect.unconsumed_event` advisory in the ScreamReport + `slog.Warn`. `DisableCoeffectValidation` escapes. Mirrored statically by cqrs-lint E018 and in the catalog export (`ValidateCoeffects` + `coeffects.md`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | 🧪     |
...
| Driver registry                 | `metaengine.RegisterDriver(name, factory)`, `metaengine.RegisteredDrivers()`, `metaengine.LookupDriver()` — database/sql model (D1). All engines self-register via `register.go` + `init()`. Wired into `New()` via `createEngineFromDriver()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🧪     |
...
| Lifecycle                       | `Close()` (joins all errors via `errors.Join`), `GracefulClose(ctx)` (drains via `Drainer`s, then context-bounded Close), `Drain(ctx)` (drains without closing, for rolling deploys), `ResetProjection(ctx, name)`, `Start(ctx)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 🧪     |
| Introspection (extended)        | `EngineNames()`, `ShutdownOrder()`, `HealthCheckDetailed(ctx)` (per-engine `[]EngineHealth`), `LagPerProjection()`, `LagDuration()`, `WorkerStatus()`, `RegisterCloser(name, closer)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 🧪     |
...
| Shutdown dependencies           | `DomainConfig.ShutdownDependencies` + `ShutdownDependency` type. `orderedEngines()` topological sort (Kahn's algorithm, cycle fallback). Projection host always closes first (not in dep edges)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 🧪     |
| Test suite                      | 60+ tests: query dispatch, driver registry, snapshot isolation + E2E, multi-decider, concurrent dispatch (20 goroutines, race detector), bus pub/sub, MultiBus, Op accessors, atomic conflict, journal, projection E2E, plan-drift, adapter serialization, HealthCheck (healthy + stopped + failed projection + unhealthy engine + SQLite ping + all engines + closed DB), GracefulClose (normal + context-expired + slow shutdown + drain timeout + close timeout + no drainers + multiple drainers), ResetProjection (no-host + positive + restart-and-replay), custom checkpoint store (deepened), orderedEngines (no-deps + basic + cycle + unknown-names + complex DAG + self-loop + duplicate edges), Close error joining + order, RegisterDrainer (called-before-close + error propagation), Drain, RegisterCloser, EngineNames, ShutdownOrder, HealthCheckDetailed, LagPerProjection, LagDuration, WorkerStatus. All pass with `-race` | ✅     |
...
| Failed-worker sentinel    | `ErrWorkerFailed` (errorfamily Infrastructure) — a worker that exhausted its restart budget is an operator action, not a Transient retry; `ErrProjectionStale` stays Transient for lag | ✅     |
...
| ClaimingTimerStore     | `scheduling/sqlstore.ClaimingTimerStore` — lease-based claiming for multi-instance dispatch on ALL THREE SQL dialects: PG CTE `FOR UPDATE SKIP LOCKED` + `UPDATE..RETURNING`, MySQL/MariaDB two-statement claim (`SELECT..FOR UPDATE SKIP LOCKED` then `UPDATE` by IDs — no `UPDATE..RETURNING` on MariaDB; requires MySQL 8.0+ / MariaDB 10.6+, older servers fail loudly at the first `Due`), SQLite single-writer. `NewClaimingMySQLStore` verified live on MariaDB 11.4 (SKIP LOCKED contention); live two-claimer contention test on PG + MySQL | ✅     |
| Claim metrics snapshot | `ClaimingTimerStore.Metrics()` returns a JSON-ready `ClaimMetricsSnapshot` (claimed batches — empty polls count as heartbeat — claimed timers, renewals, rejections), maintained internally on every committed `Due` poll and `RenewLease`; the `WithClaimMetrics` hooks stay unchanged for external exporters. A Doctor-style report or `/status` endpoint reads claim liveness with zero wiring                                                                                                                                                    | ✅     |
...
| Feature                 | Detail                                                                                                                                                                                                         | Status |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| Local DB                | `Open(dbPath)`, `OpenInMemory()` — embedded Turso Database. `file:`/`file://`-prefixed paths are normalized automatically (the driver would otherwise reject them with a cryptic I/O error — fixed 2026-09-11) | ✅     |
...
| **Typed causation on the wire** | `Metadata.Causation` (command type + ID, ADR-0031) is written to `causation_command_type`/`causation_command_id` on publish and reconstructed on consume — dedicated keys win, legacy `custom.command.*` mirrors are promoted on read (rolling upgrades work with no producer change); partial pairs and unparseable new-style IDs are rejections (issue #21; tag wave pending) | ✅     |
...
| Observational equivalence | `Interleaved(projs...)` composes projections; `AssertObservationalEquivalence` (+ `EquivalenceProbe`) proves a projection's answers are identical alone vs under another projection's interleaved events (rapid property test in-repo) | ✅     |
...
| Severity overrides | `rules.severity-overrides` (`RulesConfig.SeverityOverrides`) with fixed precedence catalog → preset → config → domain bias → `--min-severity`; invalid severities dropped WITH a warning, unknown rule IDs warned. `v5-ready` preset = sugar for `{"V007": "error"}` (2026-09-07) | ✅ |
| V007 dot-import flagging | Any dot-import of a go-cqrs-lite module fires V007 at the import position ("hides v5-removed-API usage — name the import"); non-CQRS dot-imports stay silent (F090a, 2026-09-07) | ✅ |
| Typed qualifier resolution | `analyzer.ResolveQualifierTyped` + `IsQualifierFor`/`IsEventTypeParam` — alias-fold-blindness dead for V007 and fold detection; typed answer authoritative, string scan only when type info is missing (F091 Tier 1, 2026-09-07) | ✅ |

---

### cqrs-upgrade Consumer Upgrade CLI 🔧

> `go install github.com/larsartmann/go-cqrs-lite/cmd/cqrs-upgrade/v4@latest`

| Feature               | Detail                                                                                                                  | Status |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------ |
| Pin bump to latest    | Parses a consumer `go.mod` (direct go-cqrs-lite pins only), resolves latest tags via the module proxy, rewrites offline | ✅     |
| Verification gate     | `GOWORK=off` `go mod tidy` + `build` + `vet` on the upgraded module set before reporting success                        | ✅     |
| v5 deprecation report | Runs the cqrs-lint V007 engine in-process — lists v5-removed-API usage found in the consumer's code                     | ✅     |
| `--dry-run`           | Prints the upgrade plan (current pin → target tag) without touching anything                                            | ✅     |
...
| `storage/backuptest`             | `…/storage/backuptest/v4`             | 🧪 Test harness (shared backup-lifecycle suite any backend plugs into via a Factory adapter)                                                                              |
| `transport/http`                 | `…/transport/http/v4`                 | ⚠️ Deprecated (ADR-0127; removal v5 — use go-sse / watermill)                                                                                                                                                                              |
| `transport/grpc`                 | `…/transport/grpc/v4`                 | ⚠️ Deprecated (ADR-0127; removal v5 — use watermill/ brokers)                                                                                                                                                                              |
| `prometheus`                     | `…/prometheus/v4`                     | ✅ Production                                                                                                                                                                                                                             |
| `testutil`                       | `…/testutil/v4`                       | 🧪 Test utility (`NewCmd`, `RaceEnabled` build-tag helper)                                                                                                                                    |
| `testutil/pgtestcontainer`       | `…/testutil/pgtestcontainer/v4`       | 🧪 Test utility (Postgres server DSN by priority: `DATABASE_URL` > `POSTGRES_TEST_DSN` env > Docker testcontainer > skip)                                                                      |
| `testutil/mysqltestcontainer`    | `…/testutil/mysqltestcontainer/v4`    | 🧪 Test utility (MySQL server DSN by priority: `MYSQL_TEST_DSN` env > MariaDB 11.4 testcontainer > skip — the pg pattern, third dialect)                                                       |
| `cmd/cqrs-gen`                   | `…/cmd/cqrs-gen/v4`                   | 🔧 Tool                                                                                                                                                                                                                                   |
| `cmd/api-stability`              | `…/cmd/api-stability/v4`              | 🔧 Tool                                                                                                                                                                                                                                   |
| `cmd/doc-check`                  | `…/cmd/doc-check/v4`                  | 🔧 Tool                                                                                                                                                                                                                                   |
| `stack`                          | `…/stack/v4`                          | ⚠️ Deprecated (ADR-0123, removal v5 — `system.System` is the only composition root)                                                                                                                                                        |
| `stack/memory`                   | `…/stack/memory/v4`                   | ⚠️ Deprecated (ADR-0123, removal v5)                                                                                                                                                                                                       |
| `stack/sqlite`                   | `…/stack/sqlite/v4`                   | ⚠️ Deprecated (ADR-0123, removal v5)                                                                                                                                                                                                       |
| `stack/pebble`                   | `…/stack/pebble/v4`                   | ⚠️ Deprecated (ADR-0123, removal v5; operator knobs: `WithMemTableSize`, `WithBlockCacheSize`, `WithWALBytesPerSync`, `WithPebbleCompression`)                                                                                             |
| `stack/postgres`                 | `…/stack/postgres/v4`                 | ⚠️ Deprecated (ADR-0123, removal v5; testcontainer tests pass locally)                                                                                                                                                                     |
| `stack/turso`                    | `…/stack/turso/v4`                    | ⚠️ Deprecated (ADR-0123, removal v5)                                                                                                                                                                                                       |
| `stack/duckdb`                   | `…/stack/duckdb/v4`                   | ⚠️ Deprecated (ADR-0123, removal v5; analytical OLAP, CGo required — ADR-0071)                                                                                                                                                             |
| `stack/mysql`                    | `…/stack/mysql/v4`                    | ⚠️ Deprecated (ADR-0123, removal v5; MySQL 8.0 contract tests pass, MariaDB untested)                                                                                                                                                      |
| `stack/bench`                    | `…/stack/bench/v4`                    | ⚠️ Deprecated (ADR-0123, removal v5; benchmark gate entry)                                                                                                                                                                                 |
| `stack/bbolt`                    | `…/stack/bbolt/v4`                    | ⚠️ Deprecated (ADR-0123, removal v5; durability tiers, kv store accessor)                                                                                                                                                                  |
...
| `example/goal-shaped-app`        | `…/example/goal-shaped-app`           | 💡 Demo ("The Goal in 5 minutes": declare Commands/Events/Queries, coeffect gate, config-swap test)                                                                       |
| `example/scheduler-otel-status`  | `…/example/scheduler-otel-status`     | 💡 Demo (scheduler claim observability: OTel counters + /status JSON)                                                                                                     |
| `metaengine`                     | `…/metaengine/v4`                     | 🧪 Experimental (10 engines, 10 ADTs, Record-aware folds, auto-projection, rule pipeline, materialize-vs-replay, StorageLayout, SerializablePlan, live cost measurement, layout planning)                                                 |
...
| `metaengine/sqliteengine`        | `…/metaengine/sqliteengine/v4`        | 🧪 Experimental (SQLite engine extracted from core — ADR-0115)                                                                                                                                                                            |
| `metaengine/graphadapter`        | `…/metaengine/graphadapter/v4`        | 🧪 Experimental (wraps graph.MemoryDriver as Engine — ADR-0113)                                                                                                                                                                           |
...
| `metaengine/bboltengine`         | `…/metaengine/bboltengine/v4`         | 🧪 Experimental (Bbolt LSM engine. Self-registering driver, source-of-truth tests)                                                                                                                                                        |
| `metaengine/mysqlengine`         | `…/metaengine/mysqlengine/v4`         | 🧪 Experimental (MySQL metaengine backend. Pure Go, Prober + TransactMeasurer)                                                                                                                                                            |
| `metaengine/tursoengine`         | `…/metaengine/tursoengine/v4`         | 🧪 Experimental (Turso/libSQL shim over sqliteengine. Self-registering driver)                                                                                                                                                            |
...
| `metaengine/bigtableengine`      | `…/metaengine/bigtableengine/v4`      | 🧪 Experimental (Google Cloud BigTable engine: native temporal versioned cells, counters, self-registering driver. ADR-0141. bttest-validated only)                                                                                       |
...
| `system`                         | `…/system/v4`                         | 🧪 Experimental (operator-configured CQRS topology — all 10 drivers self-register, SQLite/MySQL/Turso/Bbolt working, P0-P3 + lifecycle hardening shipped)                                 |
| `system/integration`             | `…/system/integration/v4`             | 🧪 Test suite (CGo-gated system integration tests — DuckDB/Arrow isolated so `system` consumers never need a C compiler)                                                                   |
...
| `scheduling/engine`              | `…/scheduling/engine/v4`              | 🧪 Experimental (`TimerStore[P]` facade over any `metaengine.DueClaimer` engine — ADR-0142 one-claim-stack timers; epoch-guarded MarkFired, lease-fenced Due)                                                                             |
| `claiming`                       | `…/claiming/v4`                       | ✅ Production (dialect-correct lease-claim SQL core extracted from `scheduling/sqlstore` — P0 of the durable-queue plan; timers delegate, byte-identical statements)                                                                      |
| `queue`                          | `…/queue/v4`                          | 🧪 Experimental (durable work-queue contract: `Store[T]` lifecycle/retry/DLQ/priorities/deps + `task`/`facts` subpackages; semantics from go-taskqueue's production stores; shared `queue/conformance` suite)                             |
...
### ADR-0142 Capability Matrix — engine × write-side ADT (2026-09-19)

Every first-party engine supports or refuses each write-side ADT — never silent. Refusals are data (`EngineProfile.RefusedADTs` with the reason), rendered by `Doctor` and enforced by the capability-audit universality rule (an engine that neither supports nor refuses a write-side ADT fails the audit). FactSink rides the claimkit transaction on every native engine and on the `queue/*` engines; engine-wide refusals cover it.

| Engine                           | DueClaim       | Dedup          | Mechanism / refusal reason                                  |
| -------------------------------- | -------------- | -------------- | ----------------------------------------------------------- |
| sqlite                           | native O(logN) | native O(logN) | claimkit upsert CAS                                         |
| turso                            | native O(logN) | native O(logN) | delegates to sqliteengine (libSQL driver path)              |
| postgres                         | native O(logN) | native O(logN) | claimkit CTE `FOR UPDATE SKIP LOCKED`                       |
| mysql                            | native O(logN) | native O(logN) | claimkit two-statement; lock-free dedup CAS (no gap locks)  |
| duckdb                           | native O(logN) | native O(logN) | claimkit; writes serialized by dialect-conditional mutex    |
| memory / pebble / bbolt / badger | degraded       | degraded       | `MapDueClaimer` + `MapDedupStore` runtimes (single process) |
| dgraph                           | REFUSED        | REFUSED        | DQL upserts cannot atomically fence concurrent claimers     |
| iroh (Replicated wrapper)        | REFUSED        | REFUSED        | a lease on one replica is not a lease anywhere              |
| bigtable                         | REFUSED        | REFUSED        | no atomic arbitrary-value RMW or collection scan yet        |

Full decision record: [`docs/adr/0142-universal-storage-substrate.md`](docs/adr/0142-universal-storage-substrate.md) (§ Realized capability matrix).
