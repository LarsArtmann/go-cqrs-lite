...
**Known issue:** `UpcasterRegistry.Upcast()` uses `current.Version() >= upcaster.SourceVersion()` — this may re-upcast already-current events if event version equals the upcaster's source version. Likely should be `==` for exact match. The upcaster is also responsible for setting the correct output version with no enforcement.

**Coverage:** Tested, but the `>=` edge case may not be covered
...
| --- | --- | --- |
| Metadata silently discarded | 🔴 CRITICAL | INSERT passes `nil` for metadata — all correlation IDs, user IDs, custom metadata are lost on save |
| Codec field unused | 🔴 HIGH | `WithStoreCodec` option exists but codec is never called — payloads stored/returned as raw bytes |
| `nowFunc` field unused | ⚠️ MEDIUM | Field is set but never called — dead code |
| Zero tests | 🔴 CRITICAL | No unit, integration, or benchmark tests at all |
| No Close/lifecycle | ⚠️ MEDIUM | No graceful shutdown, connection release, or lifecycle management |
...
| `core/event` | `…/core/event` | ~900 | 50+ | 99.1% | ✅ Production |
| `core/aggregate` | `…/core/aggregate` | ~250 | 27 | 95.7% | ✅ Production |
| `core/pkg/id` | `…/core/pkg/id` | ~400 | 30+ | 97.1% | ✅ Production |
| `core/pkg/dispatcher` | `…/core/pkg/dispatcher` | ~200 | 24 | 100.0% | ✅ Production |
| `memory` | `…/memory` | ~500 | Extensive | 98.9% | 🧪 Test utility |
| `catalog` | `…/catalog` | ~400 | Extensive | 94.4% | ✅ Production |
| `catalog/asyncapi` | `…/catalog/asyncapi` | ~280 | Golden-file | 97.6% | ✅ Production |
...
| `storage` | `…/storage` | ~297 | **0** | **0%** | 🔴 Broken |
