...
| Cost calibration                | `ReadCosts` per read pattern (point-lookup/filtered-scan/aggregate/scan) set by ALL 7 engines from calibration benches; read vs write cost split per engine (e.g. bbolt point lookup ~750ns vs full scan ~660ns/row; DuckDB aggregate ~150ns/row vs point lookup 50K). KV-engine `ReadAggregate` = `CounterGet` prefix scan                                                              | 🧪     |
