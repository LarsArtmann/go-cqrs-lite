      ...
      perSystem =
        ...
        in
        {
          treefmt = {
            ...
            settings.excludes = [
              "**/testdata/golden/**"
              # Generated by `templ generate` (pinned CLI v0.3.1020) —
              # gofumpt/goimports would fight the generator's output.
              "**/*_templ.go"
            ];
            programs = {
              gofumpt.enable = true;
          ...
          packages = {
            ...
            cqrs-lint = (pkgs.buildGoModule.override { go = goPkg; }) {
              ...
              vendorHash = "sha256-pjCZGc0J+byW4QwBc/mYFqPpFYp3wd9cvPeF7BfmQis=";
              proxyVendor = true;
          ...
          apps = {
            ...
            # pin-sweep: bump every sibling go.mod pin to the latest published
            ...
            build-docserver-css =
              mkApp "build-docserver-css" [ goPkg pkgs.tailwindcss_4 pkgs.gnused pkgs.diffutils ]
                ''
                  ${pkgs.bash}/bin/bash "$PWD/scripts/build-docserver-css.sh" "$@"
                '';

            check-docserver-css =
              mkApp "check-docserver-css" [ goPkg pkgs.tailwindcss_4 pkgs.gnused pkgs.diffutils ]
                ''
                  ${pkgs.bash}/bin/bash "$PWD/scripts/build-docserver-css.sh" --check
                '';
            ...
            # Usage: nix run .#ephemeral-dgraph                # start Dgraph only
