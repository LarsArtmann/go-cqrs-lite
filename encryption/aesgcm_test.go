...
func TestAES256GCM_DifferentCiphertexts(t *testing.T) {
	t.Parallel()
...
func TestAES256GCM_WrongKeyFails(t *testing.T) {
	...
	enc1, _ := encryption.NewAES256GCM(key1)
	enc2, _ := encryption.NewAES256GCM(key2)
...
func TestAES256GCM_InvalidKeySize(t *testing.T) {
	...
	_, err := encryption.NewAES256GCM([]byte("short"))
	if err == nil {
		t.Error("expected error for short key")
	}
}

func TestAES256GCM_EmptyPayload(t *testing.T) {
	t.Parallel()
...
func TestAES256GCM_TruncatedCiphertext(t *testing.T) {
	t.Parallel()
