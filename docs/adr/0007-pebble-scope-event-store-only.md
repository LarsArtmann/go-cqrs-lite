...
Should `pebble/` be expanded to cover the full `storage/` surface (Journal, SeekableJournal, CheckpointStore, SnapshotStore, Outbox)?
...
- `pebble/` may still add `CheckpointStore`, `SnapshotStore`, and `Outbox` (these are per-key operations, no global ordering needed)
