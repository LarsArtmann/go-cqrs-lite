...
- **Module path:** `github.com/larsartmann/go-cqrs-lite/event/v4/eventtest`
- **Directory:** `event/eventtest/` (mismatch!)
- **Tags:** `event/v4/eventtest/v4.3.0`, `event/v4/eventtest/v4.5.0` (v3 tags on a v0-path module)

External consumers could not resolve the module:

```
go get github.com/larsartmann/go-cqrs-lite/event/v4/eventtest
→ module github.com/.../event/v4@v3.5.0 found, but does not contain package .../event/v4/eventtest
```
...
A major-version suffix (`/v2+`) is only recognized when it is the **last** path element. For `.../event/v4/eventtest`:

- The `/v4/` is NOT the last element → it is a **literal directory name**, not a major-version suffix
- Go looks for `go.mod` at `event/v4/eventtest/go.mod`
...
1. **Move the directory** from `event/eventtest/` to `event/v4/eventtest/` so it matches the module path. This preserves ALL import paths (the module path is unchanged) — only the physical directory moves.

2. **Tag as v0:** Delete the wrong `v3.3.0`/`v3.5.0` tags. Create `event/v4/eventtest/v0.1.0` at the fix commit.
...
- External consumers can now `go get github.com/larsartmann/go-cqrs-lite/event/v4/eventtest@v0.1.0` (with `GOPRIVATE=github.com/larsartmann/*` for the private repo, or once the repo is made public).
...
- **Did NOT flatten to a top-level module.** The import path `.../event/v4/eventtest` is used by ~92 files. Moving the directory (not the path) achieves the same fix with zero import-path churn.
- **Did NOT change the module path.** The path `.../event/v4/eventtest` correctly reflects that eventtest is part of the event v3 release line, even though it's technically a v0 module.
