...
## Appendix: Execution Progress (2026-06-15)
...
### Revised Scope: Decider WithVersionedStore (T2)

T2 was originally "add WithVersionedStore option to decider." On implementation,
we realized `schema.VersionedStore` already implements `event.Store` — consumers
compose them transparently before passing to `NewRepository`. No hard dependency
or new option needed. Documented the pattern in `decider/doc.go` instead.

### What command/ and query/ Gained

```
command/:  4 → 10+ interfaces
  NEW: CommandJournal, SeekableCommandJournal
  NEW: Publisher, Subscriber, Bus, PublishMiddleware, PublisherFunc

query/:    1 → 8+ interfaces
  NEW: PersistedQuery, QuerySink, QuerySource, QueryStore
  NEW: QueryJournal, SeekableQueryJournal

event/:
  NEW: MetadataKeyCommandType, MetadataKeyCommandID
  NEW: WithCommandCausality(ctx, cmdType, cmdID)
  NEW: CommandCausalityEnricher (ContextEnricher)
  NEW: ExtractCustomBytes (shared base64 metadata extractor)

memory/:
  NEW: MemoryCommandBus (full Bus with middleware)
  NEW: MemoryQueryStore (Store + Journal)
