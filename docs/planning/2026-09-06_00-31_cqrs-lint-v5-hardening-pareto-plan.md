...
| **20%** | **Harden + prove**: severity meta-test, suppression tail, fix-provider unification, lintutil convergence, dead links, CI self-lint, perf proof, ledger hygiene | Kills whole bug *classes* (metadata split-brains across 204 rules, suppression edge cases, fix correctness) and makes the linter continuously self-checked in CI. |
