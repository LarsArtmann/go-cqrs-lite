...
4. **Respect the Declined list** (TODO_LIST §Declined): no `#verify-parallel` CI, no Redis *adapter* (Redis *bus* T22 is different and sanctioned by ADR-0123 `BusConfig`), no composite keys resurrection, no `WaitReady`.
...
*Point-in-time snapshot. Living state: TODO_LIST.md. Harvest via docs-health HARVEST after execution approval.*
