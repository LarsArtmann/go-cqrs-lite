...
| 3 | **Wrong abstraction level** — a library doesn't own consumers' identity strategy. Aggregate IDs are the *consumer's* domain concept. Forcing ULID is framework opinion, not library neutrality. |
...
The stated reason in the comment: *"Export it so domain packages can create domain-specific IDs interoperable with AggregateID."*
