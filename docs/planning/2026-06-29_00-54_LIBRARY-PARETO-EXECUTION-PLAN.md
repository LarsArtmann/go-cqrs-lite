...
| 3 | Dispatch-tier **DLQ already exists** (`middleware/deadletter.go` + `deadletter_sql.go` + tests). Gap analysis A4 was stale. | Only the *projection-tier* DLQ hook remains → folded into T6. |
