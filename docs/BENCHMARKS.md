...
| pebble durability tiers (append)         | `BenchmarkEventAppendSync`/`_Async` (`storage/pebble`)                       | `go test ./storage/pebble/ -bench 'EventAppend(Sync\|Async)$' -run XXX -benchtime 200x -count 5` (disk-backed under `$HOME/.cache/pebble-durability-bench`, override `PEBBLE_DURABILITY_BENCH_DIR`; skips on tmpfs-like setups) | fsync per append (Strict tier)         | async WAL (Normal tier): measurement PENDING quiet window — 2026-08-17 attempt was inconclusive (load 3-4 + 96%-full btrfs: raw async `Set` also ~2.5 ms/op, device queue saturated, sync ≈ async) |
