...
| **Metaengine**          | Cost-based storage planner: given a set of query declarations and available engines, assigns each query to the cheapest engine and emits cost diagnostics. Core has ZERO internal deps (stdlib + `database/sql` only).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `metaengine.Plan(engines, queries...)`, `metaengine.Store`, `metaengine.ExecuteTyped[Q,R]`, `metaengine.NewMemoryEngine()`, `metaengine.NewSQLiteEngine(db)` — ADR-0061/0062/0063                                                                       |
| **Planner**             | The decision-making component of the Metaengine: ranks engines by estimated latency (cost model: O(1) < O(logN) < O(N) < O(NlogN) < O(degree^depth)), picks the cheapest, and warns on write-amplification (one event updating too many projections).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `metaengine.PlanResult`, `metaengine.QueryAssignment`, `metaengine.Diagnostic`, `metaengine.DefaultWriteAmplificationBudget`                                                                                                                            |
