...
| **Deletion Event**   | Domain event that signals soft-delete (ADR-0114) — the blessed pattern over deprecated metadata tombstones | Emit `"entity.deleted"`; detect by event type in your handler, or via `listing.StatusMiddleware(deleteTypes, rebirthTypes)` |
| **Rebirth Event**    | Domain event that undoes a deletion                                                                        | Emit `"entity.restored"` — branch on `evt.Type()` in `OnUpdate`/fold handlers (metaengine: `metaengine.Remove`/re-add)      |
...
| Term               | Definition                                                                                                                                                                                                                                                                       | Context                                                                                                                                                                                                                                                       |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dialect**        | SQL dialect abstraction for portable store implementations. Two senses: (a) the `storage/sql` interface each SQL store takes; (b) the SERVER dialect engines detect at connect time (MySQL vs MariaDB — MariaDB lacks `->` and `CAST(? AS JSON)`, Error 1064) and render SQL for | (a) `sql.Dialect` interface in `storage/sql` — `SQLiteDialect{}`, `PostgresDialect{}` (re-exported as `storage.SQLiteDialect`); (b) `mysqlengine` `SELECT VERSION()` detection → dialect-correct JSON filter/sort forms (`metaengine/mysqlengine/dialect.go`) |
...
> **Full vocabulary:** [Metaengine Domain Language →](METAENGINE_DOMAIN_LANGUAGE.md) — a dedicated document for the cost-based storage planner's specialized terms (ADTs, fold DSL, cost model, storage layouts, read patterns, all 9 engines, key codec, projection adapter, and verification block).

A **cost-based storage planner** (CBO) for event-sourced projections. The metaengine is the strategic future of this project. It automates what the three manual projection tiers above do by hand: it infers the ADT from fold return types, picks the optimal engine, generates DDL, and routes queries — 80% auto-generated, 100% auto-routed ([ADR-0116](adr/0116-layered-auto-projection.md)). ADRs [0061](adr/0061-metaengine-sqlite-engine.md)–[0117](adr/0117-command-lifecycle-as-events.md) cover the full design.
...
| **Deriver**             | Event-to-command derivation: when event E arrives, derive and dispatch command C (ADR-0040). The reverse of the normal CQRS flow — reactions to events, not responses to commands. Fluent: `deriver.Then(cmd)`, `deriver.Filter(pred)`, `deriver.Idempotent(store)`, `deriver.AsHandler(d)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `deriver.Deriver` — `deriver.Noop`, `deriver.Then`, `deriver.Filter`, `deriver.Idempotent`, `deriver.AsHandler`                                                                                                                                         |

> **Metaengine** terms (Engine, ADT, Fold, Cost Model, Layout, PlanRule, Persistence, Replication, all engines, etc.) are defined in [METAENGINE_DOMAIN_LANGUAGE.md](METAENGINE_DOMAIN_LANGUAGE.md).
...
metaengine.Engine — full interface hierarchy in [METAENGINE_DOMAIN_LANGUAGE.md](METAENGINE_DOMAIN_LANGUAGE.md#interface-hierarchy--engine)
```
...
| "Delete"                   | "Tombstone"                    | Event streams are append-only; deletion is a domain event (ADR-0114) or a deprecated tombstone mark — never removal                       |
...
| ---------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Saga / Process Manager**   | `bus.SubscribeAll` + `command.Dispatcher` + `deriver`                          | Multi-step orchestration is domain-specific; a generic saga module imposes the wrong abstraction. See `example/taskmanager/` for a real implementation.           |
