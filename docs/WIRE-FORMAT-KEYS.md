...
| pebble events                    | CBOR   | `stream_id`, `stream_type`                                                                                     | `aggregate_id`, `aggregate_type` (decode-only)                        | no                          | v6            |
...
| event envelope `encoding` stamp  | CBOR   | typed `codec.Encoding` (open namespace: custom codecs round-trip) in pebble + bbolt event rows (E1, v5-typed)  | bare `string` rows decode identically (same wire bytes)               | n/a                         | n/a           |
