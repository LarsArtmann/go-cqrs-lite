...
4. **Harvest notes must cite verified tag contents** — the "unblocked NOW by system v4.8.0" fiction cost time; harvest should record *which symbols were checked in which tag*.
...
*(Brainstorm per status-report skill: beyond the top ~25 these are ROADMAP fuel — apply docs-health HARVEST routing rigor before promoting into TODO_LIST.)*
...
*Point-in-time snapshot. Follow-ups in (f) are HARVEST input for TODO_LIST/ROADMAP (docs-health), not commitments.*
