...
- **Formatter root cause (earlier question 2).** Symptom durably guarded
  (4/4 heals), source identified as *likely* BuildFlow's built-in golangci
...
- **F040** — making `check-lint-config` a *required* GitHub check: master
...
2. **First benchmark measurement was wrong.** I assumed two `--path`
   ...
   the tool saw what you think it saw *before* writing the doc.
...
8. **The RULES.md freshness meta-test saved the day once** (stale doc
   ...
   README preset table's *counts*, not just its rule lists.
