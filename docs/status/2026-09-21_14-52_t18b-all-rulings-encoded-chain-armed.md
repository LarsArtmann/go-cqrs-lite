...
_State at writing: cost pass pid 3244128 + closure pid 3258300 alive (waiting out load 41); scripts preserved in `/var/tmp/t18b/`; baseline `a91e7cd90` untouched; tree clean. No manual commit per harness contract. Waiting for instructions._
...
**Q1 executed — campaign queue armed** (chained after completion): SearchQuery count=5 re-run (row 306c) then a full `BenchmarkCalibration_*` measurement capture (row 306d inputs), via `nix run .#integration-dgraph`, plausibility-guarded, results + PROVENANCE to `/var/tmp/t18b/campaign-results.md`. The bench wiring was **smoke-tested first** (34s live run, PASS — the D2 lesson applied). Supersede/re-anchor _edits_ stay manual with numbers in hand (row 306d's constant campaign is a design pass, not a script).
