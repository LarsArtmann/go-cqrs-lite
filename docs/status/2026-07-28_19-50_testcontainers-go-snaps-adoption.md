...
- ✅ **`snaps.Clean(m)` added to 16 modules** — TestMain files updated across
  listing, snapshot, schema, signing, storage, storage/pebble, storage/memory,
  storage/turso, middleware, watermill, catalog sub-packages, otel, codec, and
  more. Obsolete-snapshot cleanup is now automatic.
- ✅ **`goldenFilePerm` dead constant removed** from `cattest/catalog.go`.
- ✅ **`UPDATE_SNAPS=true`/`UPDATE_SNAPS=clean` documented** in AGENTS.md
  testing section.
- ⚠️ **`TestRun_Postgres_Recovery` benchkit failure** — root cause found
  (`populateSnapshots` writes +50 events). Fixed with `SkipSnapshot: true`,
  but may still need monitoring.
