...
1. **Run `nix run .#test`** — full test suite across all modules
2. **Run `nix run .#lint`** — golangci-lint across all modules (this is a LINT branch!)
3. **Run `nix run .#verify-fast`** — build + vet + test + lint
4. **Run `nix run .#verify`** — full gate including doc-check
5. **Run `nix run .#check-arch`** — dependency budget enforcement
6. **Run `nix run .#check-duplication`** — no-new-clones gate
7. **Run `nix run .#check-coverage`** — coverage drift

### Module registration verification

8. **Verify `metaengine/bboltengine` is in `go.work`**
9. **Verify `metaengine/mysqlengine` is in `go.work`**
10. **Verify `metaengine/tursoengine` is in `go.work`**
11. **Verify `storage/backuptest` is in `go.work`**
12. **Verify all 4 new modules are in `flake.nix` testModules**
13. **Verify all 4 new modules are in `cmd/api-stability/main.go` modules list**
14. **Run meta-test `TestEveryGoModDirIsInTestModules`**
15. **Run meta-test `TestEveryGoModDirIsInModulesList`**

### API stability

16. **Regenerate API stability golden: `cd cmd/api-stability && GOWORK=off go run main.go -update`**
17. **Run doc-check: `cd cmd/doc-check && GOWORK=off go run . ../../SKILL.md ../../.agents/skills/go-cqrs-lite/references/*.md ../../AGENTS.md`**

### Conflict file investigation

18. **Check if `metaengine/layout_matrix_test.go` exists and is correct in merged master**
19. **Check if `metaengine/sqliteengine/graph.go` exists and is correct in merged master**
20. **Review `middleware/idempotency_nil_test.go` for the content conflict that was auto-merged**

### Metaengine work verification

21. **Verify `metaengine/bboltengine` tests pass standalone: `cd metaengine/bboltengine && GOWORK=off go test ./... -count=1`**
22. **Verify `metaengine/mysqlengine` tests pass standalone**
23. **Verify `metaengine/tursoengine` tests pass standalone**
24. **Verify `metaengine/pgengine` tests pass (has new probe/aggregations)**
25. **Verify `metaengine/dgraphengine` tests pass (major engine.go refactor)**
26. **Verify `metaengine/duckdbengine` tests pass (aggregations rewrite, CGo)**
27. **Run metaengine soak tests (smoke at least)**
28. **Verify `storage/backuptest` module compiles and tests pass**
29. **Verify `storage/bbolt` and `storage/pebble` import backuptest correctly**

### Codec migration verification

30. **Verify `codec/` module compiles as alias-only: `cd codec && GOWORK=off go build ./...`**
31. **Verify `codec/` tests pass (what remains): `cd codec && GOWORK=off go test ./... -count=1`**
32. **Verify consumers of `codec/` still resolve to `go-codec` correctly**
33. **Check no production code imports deleted codec symbols**

### Lint verification (the whole point of the branch)

34. **Run golangci-lint on `middleware/` — SA1019 suppressions were removed**
35. **Run golangci-lint on `cmd/cqrs-bench/` — lint-driven cleanups**
36. **Run golangci-lint on `metaengine/` — massive new code, never linted on master**
37. **Check `.golangci.yml` changes are correct**
38. **Verify no new SA1019 violations in the 24K lines of new code**

### Documentation

39. **Update AGENTS.md if module count changed (was 79 go.mod files)**
40. **Verify status reports under `docs/status/` are accurate**
41. **Check if `docs/sessions/SESSION_MILESTONES.md` needs updating**

### Cleanup

42. **Delete the `lint-sweep-recovery` branch if fully merged (it is)**
43. **Verify `go.work.sum` is clean**
44. **Run `go mod tidy` check on new modules**
45. **Verify no `.orig` or conflict marker files remain**
46. **Check `cmd/cqrs-bench` still works after lint cleanups**
47. **Verify `middleware/test_helpers_test.go` additions are correct**
48. **Review the merge commit `0e4a16a8a` — does it contain anything unexpected?**
49. **Consider whether the merge commit message is accurate ("broken snapshot")**
50. **Run `nix flake check` if Nix expressions were touched**
...
2. **Should the merge commit (`0e4a16a8a`) have been squashed instead?** The branch is now on master with a merge commit that says "restoring modules deleted by master's broken snapshot." If this is going to be in the permanent history, is that the narrative we want? Or should the history show a clean linear set of lint fixes + metaengine work without the recovery narrative?

3. **Is the `git sync` / git-town rebase workflow the standard for this repo?** If so, the merge commit approach will keep causing rebase conflicts on every sync. Should we switch to always-rebase for feature branches to avoid this class of problem in the future?
