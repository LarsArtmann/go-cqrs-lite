...
- The AGENTS.md is explicit: *"API-surface changes require golden regen in the same edit... Do NOT rely on the `#verify` gate to catch this."*
...
- The AGENTS.md rule: *"every session that changes code... must run `nix run .#verify` before claiming GREEN."* I claimed GREEN based on partial checks. **Do not trust "GREEN" without the nix gate.**
