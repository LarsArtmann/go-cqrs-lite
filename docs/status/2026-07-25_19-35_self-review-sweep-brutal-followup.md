...
|------|-------------|----------------------|
| Lint sweep (f13/14/17-19) | Fixed every issue the gate fired; lint is 0 | Did NOT proactively scan all modules for the *same class* of issue that didn't fire. The gate is reactive — I let it drive, I didn't hunt. |
...
2. **Bench/quantify the reify JSON-round-trip cost** (original list f21). I documented the *mechanism* in ADR-0066 but dropped the *measurement*. No benchmark exists.
...
1. **Track API-surface changes in real time.** Whenever I add/rename/remove an export, I must regenerate the api-stability golden *in the same edit*, not "later." The gate is a backstop, not a planner.
...
32. **Audit the 7 files I reverted from `nix fmt`** for latent drift (event/*, example/taskmanager/*) — they may still be unformatted vs the canonical style; the gate passes because lint excludes them or they happen to be clean.
