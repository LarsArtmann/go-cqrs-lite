...
1. Fix `check-arch` Layer 1: add LAYER + DEP_BUDGET entries for all 94 missing modules in `scripts/check-module-layers.sh`
2. Add `TestEveryModuleHasLayerEntry` meta-test to prevent future drift (similar to `TestEveryGoModDirIsInTestModules`)
3. Tag new releases for watermill, projectionhost (both have uncommitted API changes — the TOCTOU fix + handler independence fix are breaking behavioral changes)
4. Tag new release for storage/bbolt (godoc/param fix), stack/bbolt (constant extraction)
5. Verify standalone (`GOWORK=off`) builds still pass after the daemon commit (the 43 go.mod/go.sum files from the prior session may or may not have been committed)

### Architecture / Code Quality
6. Document the "subscribe-before-drain" TOCTOU pattern in an ADR or recipe (applies to both projectionhost and watermill CatchUpSubscriber)
...
8. Complete the `metadata.Tracing` → `record.CommonMetadata` migration to eliminate the type-mismatch split-brain
...
11. Run `art-dupl baseline` to update the duplication baseline after the `processEvent` extraction
12. Run `nix run .#check-duplication` to verify the refactor didn't create new clones
13. Add CI step for standalone (`GOWORK=off`) builds — this was identified as a gap in the prior session
...
22. Verify all module tags are monotonically increasing (the prior session created 7 new tags)
23. Run `nix run .#vulncheck` to verify standalone builds haven't regressed
24. Check if the `flightrecorder` module API changes (Recorder struct) need a new tag
25. Check if `listing` module API changes (TombstonePolicy rename) need a new tag
26. Update the `CHANGELOG.md` Unreleased section with this session's changes
27. Update `.agents/skills/go-cqrs-lite/references/*.md` if any recipes changed

### Documentation
28. Document the `errors.Join` handler error semantics in the event/bus documentation
29. Add TOCTOU race pattern to `.agents/skills/go-cqrs-lite/references/recipes.md`
...
32. The `check-coverage` drift check hasn't been run — run `nix run .#check-coverage`
...
40. Run `nix run .#check-arch` after fixing Layer 1 entries to verify enforcement actually works
...
43. Run `nix run .#check-duplication` to verify the `processEvent` extraction improved the score

### CI / Build
44. Add CI step for `GOWORK=off` per-module builds (the standalone build regression from prior session)
...
47. The pre-commit hook (BuildFlow) is broken due to missing `go-licenses` and `vulnix` — fix the nix devShell
