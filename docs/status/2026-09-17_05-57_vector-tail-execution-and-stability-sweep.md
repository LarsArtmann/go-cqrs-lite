...
| 15 | **`check-error-taxonomy` gate-script bug fixed** — claims loop read `section|dir|prefixes` from entries carrying a floor field, so the LAST prefix got corrupted (`turso_preset.|50`) and literal codes under it were silently unverifiable (`turso_preset.*`, `storage.view.*`, `listing.*` affected). Loop now reads floor/depth into own fields. Exposed + documented `storage.commit_tx`. Gate: **519 codes across 18 modules match**. | exit 0 |
...
*Point-in-time snapshot. Route section f through docs-health HARVEST; annotate,
never rewrite.*
