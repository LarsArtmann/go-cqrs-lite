...
2. **json/v2 byte-comparison audit** across all `_test.go` (grep `string(got) != string(base)`-shaped patterns); fix any further decode-boundary byte comparisons.
...
3. **`scripts/wait-for-quiet.sh`** with `--stable-minutes` (+ optional `--modules` scope) and a self-test per the gate-script convention; wire into the cascade resume runbook.
4. **Cap test-phase parallelism** in verify apps (`-p` or per-module loop) — determinism over raw speed for the canonical gate.
5. **Restore the golangci-lint cache mount** (`.golangci-disk`) — buildflow env-guard says it's dead.
6. **Investigate the render.go future-stamped mtime** (clock skew during templ regen) — one `stat` + correlation check next occurrence.
7. **HARVEST check**: items 2–7 above are TODO_LIST candidates (the 13 ADR-0141 items were already harvested by the predecessor — do not duplicate).
...
8. _(observed)_ Overnight daemon commits 00:54–00:58 swept 526 files (442+64+20; net −421 lines, mostly single-line removals) — unreviewed by me; worth a skim by its owning session.
