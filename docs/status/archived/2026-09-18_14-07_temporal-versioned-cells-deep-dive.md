...
~~1. Fix go.work duplicate `bigtableengine` entry; `go work sync`.~~ done 2026-09-18 — 16:03 §a1
~~2. Re-add the 3 depguard entries to `.golangci.yml` (re-View first; daemon race).~~ done 2026-09-18 — 16:03 §a3
~~3. `cd cmd/api-stability && GOWORK=off go run -tags "goexperiment.jsonv2" . --update` + `TestEvery`.~~ done 2026-09-18 — 16:03 §a5
~~4. Run `nix run .#lint` — expect/fix bigtableengine lint findings (funlen 30, wrapcheck, etc.).~~ done 2026-09-18 — 16:03 §a10
~~5. Run `nix run .#check-arch` — dep budget for bigtableengine (grpc/api are test-scope; verify).~~ done 2026-09-18 — 16:03 §a11
~~6. Run `nix run .#check-lint-config` (depguard verify).~~ done 2026-09-18 — check-lint-config green
~~7. Run `nix run .#check-duplication` (register.go clone annotated; verify no new clones).~~ done 2026-09-18 — 16:03 §a13
...
~~36. AGENTS.md gotchas: add "bttest vs BigTable: ms bounds, chain order, binary counters" note.~~ done 2026-09-18 — 16:03 §a14
