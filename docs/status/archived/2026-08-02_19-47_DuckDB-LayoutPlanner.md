...
DuckDB `LayoutPlanner` shipped (planned tables with extracted columns + ART indexes, engine dispatch, 8 tests). Columnar-native storage (`WithColumnarLayout`) shipped in `21-18`/`22-17`. ADR-0092 written. DuckDB float truncation fixed (float64→DOUBLE).

**Still open:** `adttest.RunMatrix` coverage for LayoutPlanner; layout backfill documentation; DuckDB layout benchmark; centralize planned-table helpers between SQLite and DuckDB. Captured in TODO_LIST.md.
