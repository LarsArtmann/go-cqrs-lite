...
> **RESOLVED + ARCHIVED (docs-health pass 2026-09-08).** The v5 follow-on
> SHIPPED the same evening as the successor session
> (`docs/status/archived/2026-09-07_20-57_turso-embedded-encryption-shipped.md`:
> `WithEncryption` + typed ciphers, live-verified). f-items 1–8, 12, 13, 37,
> 43 struck below where closed. The open remainder (upstream turso-go
> issues, `DriverConfig.Encryption` v5 ADR + KeyProvider, sync decision,
> DSN-redaction audit for sibling engines, BYOK tier docs) was harvested
> into TODO_LIST → "Metaengine — follow-ups" / v5 Unification on 2026-09-08.

**Date:** 2026-09-07 20:18 CEST
**Session scope:** Single-question session: _"Does go-cqrs-lite support Turso BYOK encryption (docs.turso.tech/cloud/encryption), and if not, how would we add it for v5+?"_ This report covers ONLY that session's work. No other research was done.
...
| -- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------ | ------------- |
| ~~1~~  | ~~Verify daemon absorbed the uncommitted 1-line `want` ordering fix; rerun `TestRedactDSN`~~ done — successor session verified committed | High   | S      | Cleanup       |
| ~~2~~  | ~~Run full tursoengine module suite (`GOWORK=off go test ./...`) after the redactDSN change~~ done — successor §a7 (full suite ×2 + -race green) | High   | S      | Quality       |
| ~~3~~  | ~~Run golangci-lint on tursoengine for the changed files~~ done — successor §a7 (0 findings on own files) | High   | S      | Quality       |
| ~~4~~  | ~~Add CHANGELOG `[Unreleased]` Fixed entry: redactDSN encryption-key leak~~ done — successor §a6 | High   | S      | Documentation |
| ~~5~~  | ~~**Fix `withExperimentalViews` silent-skip bug**…~~ done — successor §a3 (`dsn.go` `withExperimentalToken` merges comma-lists; live coexistence test) | High   | S      | Bug           |
| ~~6~~  | ~~Add regression test: local DSN with `experimental=encryption` + matViewSpecs → views flag merged, not skipped~~ done — successor §a3 (9-case table + `TestTursoEncryption_WithMaterializedViews`) | High   | S      | Quality       |
...
| ~~12~~ | ~~Pin test: `:memory:?encryption_hexkey=…` redaction…~~ done — successor §a5 | Medium | S      | Quality       |
| ~~13~~ | ~~Pin test: unparseable local DSN with query…~~ done — successor §a5 | Medium | S      | Quality       |
...
| ~~43~~ | ~~HARVEST: pull items 2-9, 15-18, 21, 25, and the matview bug (5-6) into TODO_LIST/ROADMAP per docs-health~~ done 2026-09-08 docs pass | High   | S      | Documentation |
