...
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------- | -------------- |
| ~~ 1 ~~  | ~~ Fix the RED `check-file-size` ratchet on `metaengine/store.go` (945→954 formatter reflow, daemon commit `9b53373ac`, pre-session): owner picks shrink-vs-policy (see 29/30) ~~ done 2026-09-16 — 15-02, CHANGELOG                                 | ~~ High ~~ | ~~ S ~~  | ~~ CI ~~       |
...
| ~~ 9 ~~  | ~~ Verify cqrs-lint V007 v5-deprecation report now lists `Infer`/`InferFromNamedEvents`/`On`/`OnTyped` (marker-driven? run it) ~~ done 2026-09-16 — 15-02 §a10; CHANGELOG                                                                            | ~~ Med ~~  | ~~ S ~~  | ~~ Quality ~~  |
...
| ~~ 12 ~~ | ~~ Carryover (09-15): drive `TestRecipesCompile`'s 12 failing snippet packages to zero (queue of researched fixes exists in that report §b1) ~~ done 2026-09-15/16 — driven to zero, verified green                                                  | ~~ High ~~ | ~~ M ~~  | ~~ Docs ~~     |
| ~~ 13 ~~ | ~~ Carryover (09-15): `check-file-size` verification of `recipes_catalog_meta.go` (344 — under cap today; confirm and tick that item) ~~ done 2026-09-16 — split; ratchet green                                                                      | ~~ Low ~~  | ~~ XS ~~ | ~~ CI ~~       |
| ~~ 14 ~~ | ~~ Index this report + the two 09-15 reports in `docs/status/README.md` ~~ done — 7th pass + 08-05 §f16                                                                                                                                              | ~~ Low ~~  | ~~ XS ~~ | ~~ Docs ~~     |
| ~~ 15 ~~ | ~~ ANNOTATE the 08-47 audit report §f items 4/5/8/9/10 with done-markers citing commits (or decide §f stays unannotated as policy) ~~ done 2026-09-16 — 7th pass                                                                                     | ~~ Low ~~  | ~~ S ~~  | ~~ Docs ~~     |
...
| ~~ 25 ~~ | ~~ Confirm comment-only deprecation doesn't shift the api golden: run `cmd/api-stability` check (analyzed as safe; not run) ~~ done — §a6: golden records none, verified                                                                             | ~~ Low ~~  | ~~ XS ~~ | ~~ CI ~~       |
...
| ~~ 29 ~~ | ~~ store.go shrink path: extract an `applyIdempotentWithRecord(...)` helper to reclaim ≥9 formatter-stable lines (only if owner picks shrink over policy) ~~ done 2026-09-16 — applyIdempotentFeed                                                   | ~~ Med ~~  | ~~ S ~~  | ~~ Refactor ~~ |
| ~~ 30 ~~ | ~~ store.go policy path: teach check-file-size to ignore pure-reformat growth (diff-shape-aware) or document the bump-baseline exception in AGENTS contract #1 ~~ done 2026-09-16 — resolved via shrink; ratchet green                               | ~~ Med ~~  | ~~ M ~~  | ~~ CI ~~       |
...
| ~~ 36 ~~ | ~~ TODO_LIST policy: this repo keeps `[x]` items with evidence; docs-health deletes done items — pick ONE convention (today both happened: I ticked, the parallel session deleted) ~~ done — convention: delete done rows to CHANGELOG (TODO header) | ~~ Med ~~  | ~~ XS ~~ | ~~ Process ~~  |
