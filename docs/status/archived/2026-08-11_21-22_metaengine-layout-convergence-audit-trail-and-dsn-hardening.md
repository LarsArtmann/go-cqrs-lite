...
4. **Fix the `gopls slicesbackward` hint** on `plan_audit.go:112` (or document why the indexed loop is intentional — the AGENTS.md documents the `slices.Backward` copy footgun).
...
32. **Add a `CONTRIBUTING.md` note** about the layout matrix test (how to update it when recalibrating).
33. **Verify `planQuery`'s `resolvePriority` call uses `meta.QueryConfig()` not `cfg`** — I need to double-check I passed the right config.
34. **Add a fuzz test for `replaceDBInDSN`** — random DSN strings.
35. **Consider making `maxPlanHistory` configurable** (option on Plan).
36. **Add `WithPlanHistoryLimit(n)` option** for operators who want more/fewer audit entries.
37. **Profile the audit trail under high replan frequency** — ensure the ring buffer doesn't allocate.
38. **Add a `ReplanLayout` test that verifies the `From` field** matches the plan's actual layout (not Embed).
39. **Document the trigger taxonomy** in the ADR.
40. **Add a `Store.ReplanReason()` method** returning the last trigger (convenience for dashboards).
41. **Consider emitting an OTel event** on each replan with the trigger.
42. **Add a `PlanHistory()` test for the bound** (push 40 entries, verify only last 32 retained).
43. **Review whether `clonePriorityConfig` is needed** — the PriorityConfig is small; maybe value semantics suffice.
44. **Add a `Doctor()` test for the `audit:` line format** (parse the `←`-separated chain).
45. **Consider adding layout to `Explain(ctx, queryName)`** output (per-query, not just Doctor).
46. **Run `nix run .#vulncheck`** — per-module standalone build (catches version-sequence breaks). <- OPEN. TODO_LIST 'Release / Tagging' (pre-tag checklist)
