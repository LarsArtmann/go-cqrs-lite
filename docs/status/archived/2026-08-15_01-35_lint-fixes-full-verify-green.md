...
3. After tags: `go mod tidy` sweep of the ~49 stale `// indirect` shim refs <- OPEN. TODO_LIST 'Release / Tagging' (~49 stale indirect refs)
   ...
   dirs (e4).
4. Run `nix run .#vulncheck` + `nix run .#check-arch` as pre-tag release <- OPEN. TODO_LIST 'Release / Tagging' (pre-tag checklist)
...
5. cqrs-lint per-module regression tests — carried. <- OPEN. TODO_LIST 'cqrs-lint' (per-module regression tests)
6. `.golangci.yml` exclusion audit (all exclusions justified + minimal, <- OPEN. TODO_LIST 'Code Quality' (.golangci.yml exclusion audit)
   including the one added this session) — carried.
7. DuckDB/Row calibration benchmarks — carried. <- OPEN. TODO_LIST 'Metaengine' (calibration benchmarks)
8. Real Redis/NATS broker roundtrips for the watermill adapter — carried. <- OPEN. Redis roundtrip shipped at d8c73be0a; NATS + broker edges = TODO_LIST 'Code Quality' (Wire broker tests into CI)
9. v5 Phase 8: delete transport/http, transport/grpc, and deprecated <- OPEN. TODO_LIST 'v5 Unification Phase 8'
   shells; write the migration guide — carried.
10. go-codec repo scaffolding — carried.
11. v5 decision: keep the kvstore SA1019 exclusion permanently or migrate <- OPEN. TODO_LIST 'v5 Unification Phase 8' (kvstore SA1019 decision item)
