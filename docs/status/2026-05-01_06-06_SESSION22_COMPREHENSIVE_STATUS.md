...
| Item | Detail |
|------|--------|
| **Storage coverage reported as 63.5%** | Previous sessions reported 79.8% → 92.3%. The number dropped because `SQLCheckpointStore` and `SQLSnapshotStore` were added (59 + 154 = 213 lines) with zero tests. The event store itself is 96.2% — the low number masks that the *new code* is the gap, not the old code. |
