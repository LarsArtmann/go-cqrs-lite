...
| 4   | ~~**Projection parallelism**~~ (`WithParallelProjections()`)                              | DiscordSync (Missing #1)                                  | **RESOLVED:** Already the default. Each projection always ran in its own goroutine with independent checkpoint. Feedback was a misunderstanding. |
...
| ~~7~~ | ~~**Projection parallelism**~~ — `WithParallelProjections()`                         | ~~DiscordSync: slow projections block fast ones.~~ Already the default.                | ~~resolved~~ |
