...
The user asked: *"How can we make it easier for people (especially AI) to use this lib to its full potential?"* The answer was a **dedicated SKILL.md** — a single-source AI consumer guide that replaces the need to discover and read 28 module READMEs. This session built that guide, cross-linked the entire module graph, fixed the stale docs index, and — critically — caught and fixed **11 incorrect API signatures** that would have broken any consumer trusting the initial draft.
