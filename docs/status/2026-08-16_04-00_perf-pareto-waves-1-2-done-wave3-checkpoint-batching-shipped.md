...
- **F11 verify-fast**: first run executed while I was mid-edit on projectionhost (see (d)),
  ...
  end-to-end verify-fast re-run still owed.
- **F14 lint gate**: exhaustruct fix is compile+test green, but `nix run .#lint` itself not re-run.
- **F31 docs for checkpoint batching**: recipes.md note + modules.md projectionhost row MISSING.
...
- F24–F27 tag chain ([BLOCKED:user] — now a FOUR-module chain, see Q1).
- Wave 3 remainder: F32–F34 bbolt `db.Batch` opt-in path; F35–F36 PG COPY spike+bench; F37–F38
  ...
  Wave 3/4 leftovers.
- All of Wave 4 (F41–F60).
- `-race` pass on projectionhost with the new mutex-guarded state.
...
1. Regen api-stability golden (+`WithCheckpointEvery`/`WithCheckpointInterval`).
2. `nix run .#lint` (F14 close-out).
3. Frozen-tree `nix run .#verify-fast` end-to-end (F11 proper).
4. `nix run .#check-coverage` (F12); add tests for uncovered new code.
5. `projectionhost` `-race -count=3` on the new checkpoint state.
6. F31: recipes.md §readmodels checkpoint-tuning note.
7. modules.md projectionhost row: + both checkpoint options.
8. CHANGELOG entry for checkpoint batching (Wave 3 ship note).
9. Coordinate on `TestSystem_ResetProjection_RestartAndReplay` with the concurrent session
   (my baseline-proof at `f836c7f1c` is useful input; their fixes are staged, not mine to touch).
10. Re-run `#check-duplication` + `#check-arch` after the above (pre-tag standing gates).
11. [BLOCKED:user Q1] Tag `metadata/v4 v4.5.0` (BrandedString/ActorString).
12. [BLOCKED:user Q1] Tag `event/v4 v4.6.1` (ReconstructEventWithMetadata).
13. [BLOCKED:user Q1] Tag `storage/v4 v4.7.0` (batch_size exports + packet-safe chunking).
14. [BLOCKED:user Q1] Tag `projectionhost/v4` next patch (checkpoint options).
15. [BLOCKED:user] Bump command/query/event pins → metadata v4.5.0 (standalone builds).
16. [BLOCKED:user] Bump storage/pebble + storage/bbolt pins → event v4.6.1.
17. [BLOCKED:user] Bump stack/mysql pin → storage v4.7.0.
18. [BLOCKED:user] `#vulncheck` (per-module standalone builds resolve the new APIs).
19. F32: bbolt `WithBatchCommit` design; read `db.Batch` fn-retry semantics.
20. F33: bbolt opt-in batch-write path (single-writer default unchanged).
21. F34: bbolt batch tests (concurrent writers → identical journal; fn retry-idempotence).
22. F35: pgx COPY FROM spike behind interface (stream_log backfill only).
23. F36: COPY vs multi-VALUES bench at 10K/100K rows; keep winner behind option.
24. F37: pebble `WithMemTableSize/WithCacheSize/WithWALBytesPerSync/WithCompression`.
25. F38: wire knobs through stack/pebble; defaults byte-identical test.
26. F39: `docs/BENCHMARKS.md` perf ledger.
27. F40: harvest Wave 3/4 leftovers into TODO_LIST.
28. F41: bbolt deserialize benchmark (extrapolated → measured).
29. F42: `reconstructEventAdopt` design + equivalence/race tests.
30. F43: wire adopt path into pebble+bbolt; bench delta.
31. F44: idempotencyTracker TTL bound option (default-unbounded decision documented).
32. F45: tracker eviction + 1M-ID memory-bound test.
33. F51: fix ignored `MarshalMetadataJSON` error (`system/adapter_event_serial.go:31`).
34. F50: SQLite durability tier outside WAL guard (`stack/sqlite/preset.go:243`) + Relaxed≠FULL test.
35. F52: ScanSlice RowCount pre-size; Custom map size hints.
36. F47: benchstat baselines committed; contention benches at `-cpu=16,32`.
37. F48: measure-then-pad worker counters @32P (pad only if >10% delta; record either way).
38. F49: measure-then-pad `multiSeqCounter` + `SSEReplay.seq`.
39. F53: pin-drift meta-test (required-vs-latest tags, fail on staleness).
40. F54: `system/integration` DuckDB standalone replace-directive/driver guard.
41. F55: `DecorateJournal` for VersionedSeekableJournal (ADR-0126 completion).
42. F56: singleflight leader-ctx capture fix (`decider/load.go`).
43. F57: turso DSN leak in errors (`register.go:69`).
44. F58: sqliteengine/tursoengine Close() leak.
45. F59: seq-carrying journal reads — `StreamLogEntry{Seq,Value}` design.
46. F60: engine capability conformance test skeleton.
