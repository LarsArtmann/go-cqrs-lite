...
2. **Run `go mod tidy -e` in every cmd/* and stack/* module** — the missing `go.sum` entries for `stack/bbolt/v4`, `stack/mysql/v4`, etc. block isolated GOWORK=off builds in `cmd/cqrs-bench`.
