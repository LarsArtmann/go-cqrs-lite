...
- Decisions still open: ~~`command.rejected` event (T17 deferred half)~~ (SHIPPED 2026-09-15 — T17 option A, `DefaultRejectionFamilies` + `RejectionLog`), session-log boundary (T18 — routed 2026-09-21: `LarsArtmann/cqrs-htmx#25` + ROADMAP OQ #15), query-level `Stream(ctx, input, fn)` (T16 follow-up — routed: ROADMAP OQ #13).
