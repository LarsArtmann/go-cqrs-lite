...
- **TODO said "touches signing/encryption hot paths"** — FALSE. Searched all callers: signing and encryption use `event.WithCustom` *option* (which internally calls `EnsureCustom`), not `EnsureCustom` directly. No direct coupling. The migration was safe and mechanical.
