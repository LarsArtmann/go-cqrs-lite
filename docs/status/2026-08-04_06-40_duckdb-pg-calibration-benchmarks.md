...
This is philosophically defensible (the cost constant models the *intended* batch workload, not the current per-row API), but it means the constant describes a capability the engine doesn't have. **The engine should have a `MapBatchSet` method, or the constant should acknowledge it models a future API.**
