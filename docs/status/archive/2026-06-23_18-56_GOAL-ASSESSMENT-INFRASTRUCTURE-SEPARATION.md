...
**Goal:** _"Consumers should NOT decide on infrastructure — deployer chooses where data lives. Library should have recommendations (which DB fits which concern). Must support SQLite + Memory, ideally multiple SQLite DBs (1 for Command+ES, 1 for Query, 1 for views)."_
