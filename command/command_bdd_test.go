	...
	Describe("As a developer creating commands", func() {
		Context("when I create a command with valid inputs", func() {
			It("should populate all fields correctly", func() {
				...
				Expect(cmd.Metadata().ActorID.Raw()).To(Equal(userID.String()))
		...
		Context("when I create a command with a zero stream ID", func() {
			It("should reject my input and explain that the stream ID is required", func() {
