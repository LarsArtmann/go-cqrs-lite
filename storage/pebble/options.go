...
func DefaultOptions() *pebble.Options {
	opts := &pebble.Options{}
heckpoint lookups). 10 bits per key gives ~1% FPR.
	filterPolicy := bloom.FilterPolicy(bloomFilterBitsPerKey)
	opts.Levels = make([]pebble.LevelOptions, 7)

	for i := range opts.Levels {
		opts.Levels[s.Levels[i].FilterType = pebble.TableFilter
ve write throughput on multi-core systems.
	opts.MaxConcurrentCompactions = func() int { return 4 }

	return opts
}

// DefaultOptionsWithLogging returns DefaultOptions with an Event write-stall events at Debug level.
//
//	backend, _ := pebble.Open(dir, pebble.unc DefaultOptionsWithLogging(logger *slog.Logger) *pebble.Options {
	opts := DefaultOptions()
