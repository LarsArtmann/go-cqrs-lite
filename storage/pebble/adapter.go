...
func (adapter *KVAdapter) Has(_ context.Context, key []byte) (bool, error) {
	...
	//cqrs-lint:ignore(C015) pebble closer releases iterator after point lookup
	_ = closer.Close()
...
func (adapter *KVAdapter) Set(_ context.Context, key, value []byte) error {
	...
	return wrapInfraOrOK(
		adapter.database.Set(key, value, adapter.writeOptions()),
		"pebble.adapter.set",
...
func (adapter *KVAdapter) Delete(_ context.Context, key []byte) error {
	...
	return wrapInfraOrOK(
		adapter.database.Delete(key, adapter.writeOptions()),
		"pebble.adapter.delete",
