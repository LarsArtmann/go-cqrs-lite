...
func (s *SQLEventStore) LoadStream(ctx context.Context, ref event.AggregateRef) (event.EventStream, error) {
	...
	query := fmt.Sprintf(`SELECT id, event_type, aggregate_type, aggregate_id, version, schema_version, payload, metadata, occurred_at
		FROM `+sqlpkg.TableEvents+` WHERE aggregate_type = %s AND aggregate_id = %s ORDER BY version ASC`, p1, p2)
...
func (s *sqlEventStream) Next() (event.Event, bool) {
	if s.err != nil { return nil, false }
	if !s.rows.Next() { return nil, false }
	evt, err := s.store.scanEvent(s.rows)
	if err != nil { s.err = err; return nil, false }
	return evt, true
}
