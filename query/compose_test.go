...
func TestCompose_MultipleErrors(t *testing.T) {
	...
	combined := query.Compose(err1, err2, err3)
...
func TestCompose_WithClassifiedErrors(t *testing.T) {
	...
	rejection := query.NewRejection("query.rejected", "rejected")
	infrastructure := query.NewInfrastructure("query.infra", "infra failed")

	combined := query.Compose(rejection, infrastructure)
	if combined == nil {
...
func TestCompose_MixNilAndErrors(t *testing.T) {
	...
	combined := query.Compose(nil, err1, nil)
