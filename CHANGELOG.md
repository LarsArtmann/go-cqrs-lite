...
- **E019 sees variable-passed data products**: same-file single-indirection resolution (a product variable passed to the registry Add call) now registers for the contract-completeness check (dispatch + scanner, positive + negative tests).
...
Coordinated release of the full 2026-09-08 → 09-19 surface (92 modules):
`record/v4.5.1`, `id/v4.6.1`, `dedup/v4.2.2`, `dispatcher/v4.4.1`, `kv/v4.3.1`, `event/v4.11.1`, 
`command/v4.11.0`, `query/v4.8.1`, `metadata/v4.7.1`, `scheduling/v4.5.0`, `claiming/v4.0.0`, 
`schema/v4.4.1`, `snapshot/v4.5.1`, `projection/v4.4.0`, `deriver/v4.3.1`, 
`commandlifecycle/v4.2.0`, `idempotency/kvstore/v4.3.0`, `otel/v4.5.0`, `otel/otlp/v4.0.0`, 
`prometheus/v4.3.1`, `middleware/v4.6.1`, `signing/v4.3.1`, `encryption/v4.4.1`, `testutil/v4.3.1`, 
`testutil/pgtestcontainer/v4.2.1`, `watermill/v4.6.1`, `transport/http/v4.3.2`, 
`transport/grpc/v4.3.1`, `metaengine/v4.14.0`, `storage/v4.10.0`, `storage/memory/v4.5.2`, 
`storage/pebble/v4.4.1`, `storage/bbolt/v4.2.1`, `storage/turso/v4.3.2`, 
`storage/backuptest/v4.2.1`, `decider/v4.7.0`, `graph/v4.3.1`, `listing/v4.4.1`, 
`projectionhost/v4.5.0`, `queue/v4.0.0`, `scheduling/sqlstore/v4.1.0`, 
`metaengine/sqliteengine/v4.4.0`, `metaengine/pgengine/v4.4.0`, `metaengine/mysqlengine/v4.3.0`, 
`metaengine/duckdbengine/v4.3.0`, `metaengine/badgerengine/v4.3.0`, 
`metaengine/bboltengine/v4.3.0`, `metaengine/pebbleengine/v4.4.0`, 
`metaengine/dgraphengine/v4.3.0`, `metaengine/irohengine/v4.3.0`, 
`metaengine/irohengine/loopback/v4.0.3`, `metaengine/irohengine/quic/v4.2.1`, 
`metaengine/bigtableengine/v4.0.0`, `metaengine/otelobserver/v4.0.0`, 
`metaengine/projectionadapter/v4.5.0`, `metaengine/graphadapter/v4.1.1`, `queue/sqlite/v4.0.0`, 
`queue/postgres/v4.0.0`, `queue/mysql/v4.0.0`, `scheduling/engine/v4.0.0`, 
`idempotency/sqlstore/v4.4.0`, `commandlifecycle/projections/v4.2.0`, `scenario/v4.4.0`, 
`metaengine/tursoengine/v4.2.0`, `metaengine/bench/v4.1.0`, `integration/v4.2.1`, 
`benchkit/v4.6.0`, `stack/v4.4.1`, `stack/memory/v4.4.1`, `stack/sqlite/v4.3.2`, 
`stack/duckdb/v4.2.1`, `stack/pebble/v4.4.1`, `stack/bbolt/v4.2.1`, `stack/postgres/v4.4.1`, 
`stack/mysql/v4.2.1`, `stack/turso/v4.4.1`, `stack/bench/v4.3.0`, `system/v4.8.0`, 
`system/integration/v4.0.0`, `catalog/v4.5.0`, `cmd/cqrs-lint/v4.12.0`, `cmd/cqrs-bench/v4.3.1`, 
`cmd/cqrs-upgrade/v4.1.0`, `cmd/cqrs-gen/v4.3.1`, `cmd/doc-check/v4.3.1`, 
...
- **`RulesConfig.SeverityOverrides`** (`rules.severity-overrides` in
  `.cqrs-lint.json`): rewrite a rule's catalog severity per project, e.g.
  `{"V007": "error"}` makes v5-removed-API findings block CI without
  disabling anything. Fixed precedence: catalog → preset → parent config →
  local config → domain bias → `--min-severity` (filter only). Invalid
  severity names are dropped with a warning (the rule keeps its catalog
  severity — nothing silently demotes to info); unknown rule IDs warn.
- **`PresetV5Ready`** (`--preset v5-ready`): the opt-in escalation for a
  completed (or deadline-bound) v5 migration — sugar for exactly the V007→
  error override. Disables nothing; no feature pins; generated by
  `cqrs-lint init`, rendered in `explain`, `doctor` (text + JSON), and the
  README preset table (test-locked).
- **V007 flags dot-imports** (`F090a`): a dot-import of any go-cqrs-lite
  module fires V007 at the import position — dot-imports hide
  v5-removed-API usage from selector attribution, exactly during a v5
  migration. Non-CQRS dot-imports stay silent.
- **Typed qualifier resolution** (`ResolveQualifierTyped`, F091 Tier 1):
  V007 resolves selector qualifiers through the type checker
  (`TypesInfo.Uses`) with a string-scan fallback only when type info is
  unavailable, killing the shadowed-qualifier false-positive class by
  construction (a local value named like the package no longer
  double-fires). Adoption measured on the repo-root corpus: no visible
  wall-time regression (medians 14.7s → 12.4s under load; the loader has
  carried `NeedTypes` since day one) —
  `docs/benchmarks/2026-09-07_cqrs-lint-f091-tier1-typed-qualifier.md`.

### Fixed — cqrs-lint self-lint gate + doctor determinism — 2026-09-07

- **Stale C025 suppression removed from `cmd/cqrs-lint/run.go`** — left
  over from the refactor that added `%w` to the stale-suppressions error
  (C025 correctly stopped firing); the directive made the CI
  `cqrs-lint-self-lint` job red on master. Zero findings otherwise; the
  job is green again.
...
- **doctor determinism**: the primary-module profile selection
  ...
  panel also honors `Monetary` pins, and doctor (text + JSON) renders
  `rules.severity-overrides`.
- **Load-robust timing tests**: `benchkit` hang ceilings and `system/v4`
