...
// filterAppendInBlock checks whether a block contains the pattern:
// if-statement → body → append() call. Returns the position if found.
func filterAppendInBlock(body *ast.BlockStmt) (token.Pos, bool) {
...
func sliceHasPaginationVar(expr ast.Expr) bool {
	...
	ast.Inspect(expr, func(n ast.Node) bool {
		...
		for _, pv := range paginationVarNames {
			if name == pv {
				found = true
				return false
			}
