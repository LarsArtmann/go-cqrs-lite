...
// projectHasCallByName reports whether any non-test file calls a function named
// funcName, regardless of package qualifier. Useful for detecting calls like
// NewRepository where the package qualifier varies.
func projectHasCallByName(ctx *analyzer.AnalysisContext, funcName string) bool {
	for _, gf := range ctx.GoFiles {
		if gf.IsTest {
			continue
		}
