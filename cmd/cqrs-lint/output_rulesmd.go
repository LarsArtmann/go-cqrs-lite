...
// markdownCategoryTitle renders category names as section headings.
var markdownCategoryTitle = map[string]string{
...
func categoryTitle(c string) string {
	if t, ok := markdownCategoryTitle[c]; ok {
		return t
	}
...
func renderRulesMarkdown() (string, error) {
	all := rules.AllRules()
	...
	for _, c := range markdownCategoryOrder {
		if len(byCat[c]) > 0 {
			cats = append(cats, c)
		}
	}
	...
	b.WriteString("<!-- Regenerate: cd cmd/cqrs-lint && GOWORK=off go run -tags \"goexperiment.jsonv2\" . rules --markdown > RULES.md -->\n\n")
