...
if [ -n "$(staged 'go\.mod$')" ]; then
	echo "==> Version drift check (commit touches go.mod)"
	if ! bash scripts/check-version-drift.sh; then
		echo "ERROR: module version pins drifted. Run: nix run .#pin-sweep"
		exit 1
	fi

	echo "==> Replace-directives check (commit touches go.mod)"
	if ! bash scripts/check-replace-directives.sh; then
		echo "ERROR: replace directives are not sibling-local. Run: nix run .#check-replace-directives"
		exit 1
	fi
fi

if [ -n "$(staged 'flake\.nix$|^scripts/check-module-layers\.sh$')" ] ||
	[ -n "$(git diff --cached --name-only --diff-filter=AD -- '*/go.mod' | head -1)" ]; then
	echo "==> Module-layers check (flake.nix / layer script / module set changed)"
	if ! bash scripts/check-module-layers.sh; then
		echo "ERROR: module layer classification drifted. See scripts/check-module-layers.sh"
		exit 1
	fi
fi
