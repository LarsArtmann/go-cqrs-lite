...
// TestCrossEngineSortedMapParity — MapScan with a sort function produces
// identical ordered results across memory and SQLite engines.
func TestCrossEngineSortedMapParity(t *testing.T) {
	t.Parallel()
	...
	items := []map[string]any{
		{"id": "c", "priority": float64(3)},
		{"id": "a", "priority": float64(1)},
		{"id": "e", "priority": float64(5)},
		{"id": "b", "priority": float64(2)},
		{"id": "d", "priority": float64(4)},
	}
	...
	results := make(map[string][]any, len(engines))
	...
	for name, eng := range engines {
		t.Run(name, func(t *testing.T) {
			...
			if !ok {
				t.Fatalf("%s engine does not implement MapBackend", name)
			}

			sb, ok := eng.(metaengine.ScanBackend)
			if !ok {
				t.Fatalf("%s engine does not implement ScanBackend", name)
			}

			for _, item := range items {
				if err := mb.MapSet(ctx, "sorted_items", item["id"], item); err != nil {
					t.Fatalf("MapSet: %v", err)
				}
			}

			got, err := sb.MapScan(ctx, "sorted_items", nil, sortFunc, nil, 0)
			if err != nil {
			...
			if len(got) != len(items) {
				t.Fatalf("%s: got %d items, want %d", name, len(got), len(items))
			}

			for i, v := range got {
				m, _ := v.(map[string]any)

				expectedPriority := float64(i + 1)
				actual, _ := m["priority"].(float64)

				if actual != expectedPriority {
					t.Fatalf("%s: position %d has priority %v, want %v",
						name, i, actual, expectedPriority)
				}
			}
		})
	}

	assertCrossEngineSliceEq(t, "SortedMap scan results", results)
}
