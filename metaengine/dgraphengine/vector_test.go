...
// TestVectorRoundtrip exercises the brute-force VectorBackend on a live
// Dgraph server (v24+, float32vector predicates): insert, k-NN ordering per
// metric, distance parity, filtered k-NN, counter, and reset.
func TestVectorRoundtrip(t *testing.T) {
	...
	if results[0].Distance != metaengine.VectorDistance(query, []float32{1, 0}, "cosine") {
		t.Fatalf("top distance = %v, want %v",
			results[0].Distance, metaengine.VectorDistance(query, []float32{1, 0}, "cosine"))
	}
...
// including the delete-mutation that clears stale metadata.
