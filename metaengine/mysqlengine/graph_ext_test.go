...
func TestMySQLGraph_RemoveEdge(t *testing.T) {
	t.Parallel()

	gb := mustNewMySQLEngine(t).(graphBackend)
	gx := gb.(graphExtBackend)
	ctx := context.Background()
	col := "test_graph_mysql_remove"
...
func TestMySQLGraph_NeighborsUndirected(t *testing.T) {
	t.Parallel()

	gb := mustNewMySQLEngine(t).(graphBackend)
	gx := gb.(graphExtBackend)
	ctx := context.Background()
	col := "test_graph_mysql_undirected"
...
func TestMySQLGraph_UndirectedAfterRemoval(t *testing.T) {
	t.Parallel()

	gb := mustNewMySQLEngine(t).(graphBackend)
	gx := gb.(graphExtBackend)
	ctx := context.Background()
	col := "test_graph_mysql_und_rm"
...
func TestMySQLGraph_OptionalCapabilityChecks(t *testing.T) {
	t.Parallel()

	eng := mustNewMySQLEngine(t)

	if !metaengine.HasGraphEdgeRemoval(eng) {
		t.Error("mysqlEngine should implement GraphRemoveEdge")
	}

	if !metaengine.HasUndirectedGraphSupport(eng) {
		t.Error("mysqlEngine should implement GraphNeighborsUndirected")
	}
}
