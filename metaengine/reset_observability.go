...
func (s *Store) resetDoctorSection() string {
	...
	for _, eng := range engines {
		...
		fmt.Fprintf(&b, "  %s: NOT bulk-clearable (no EngineResetter) — Store.Reset will be partial\n", name)
		unclearable = true
	}

	if unclearable {
		b.WriteString("  remedy: route those collections to a reset-capable engine or clear its data out-of-band before replaying\n")
	}
