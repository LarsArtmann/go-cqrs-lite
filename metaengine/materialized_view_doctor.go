	...
	for _, eng := range engines {
		...
		for _, info := range reporter.MaterializedViews() {
			...
			if info.Spec.GroupBy != "" {
				fmt.Fprintf(
					&b,
					"    WARN: grouped views return silently wrong aggregates once a group is updated by a second transaction (turso-go <= v0.8.0-pre.8, verified 2026-09-07); scalar views are the safe shape\n",
				)
			}
